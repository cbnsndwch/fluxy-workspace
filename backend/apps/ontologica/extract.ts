...
async function pipelineLLM(
    ...
    const onRetry = ctx
        ? (event: RetryEvent) => {
              ...
              if (h['retry-after'])
                  headerParts.push(`retry-after: ${h['retry-after']}s`);
              if (h['x-ratelimit-limit-requests'])
                  headerParts.push(
                      `req limit: ${h['x-ratelimit-limit-requests']}`
                  );
              if (h['x-ratelimit-remaining-requests'])
                  headerParts.push(
                      `req remaining: ${h['x-ratelimit-remaining-requests']}`
                  );
              if (h['x-ratelimit-limit-tokens'])
                  headerParts.push(
                      `tok limit: ${h['x-ratelimit-limit-tokens']}`
                  );
              if (h['x-ratelimit-remaining-tokens'])
                  headerParts.push(
                      `tok remaining: ${h['x-ratelimit-remaining-tokens']}`
                  );
              if (h['x-ratelimit-reset-requests'])
                  headerParts.push(
                      `req resets: ${h['x-ratelimit-reset-requests']}`
                  );
              if (h['x-ratelimit-reset-tokens'])
                  headerParts.push(
                      `tok resets: ${h['x-ratelimit-reset-tokens']}`
                  );
              const headerStr =
              ...
              ctx.log(
                  ctx.stage,
                  'warn',
                  `Rate limited (${event.status}) — waiting ${waitSec}s`,
...
function getActiveBaseLayerItems(
    db: Database.Database,
    projectId: number
): BaseLayerItem[] {
    return db
...
function buildBaseLayerContext(items: BaseLayerItem[]): string {
    ...
    ctx +=
        'If a concept matches a base layer class, use its exact name and URI.\n';
    ctx += 'If a concept is a specialization, note the parent base class.\n\n';
    if (classes.length > 0) {
        ctx +=
            'Classes:\n' +
            classes
                .map(c => `- ${c.local_name || c.label} (${c.uri})`)
                .join('\n') +
            '\n';
    }
    if (properties.length > 0) {
        ctx +=
            'Properties:\n' +
            properties
                .map(p => `- ${p.local_name || p.label} (${p.uri})`)
                .join('\n') +
            '\n';
    }
    return ctx;
  ...
  `)
    ...
    const activeLayerIds = new Set(
        (
            db
                .prepare(
                    'SELECT layer_id FROM onto_project_layers WHERE project_id = ?'
                )
    ...
    const allLayers = db
        .prepare('SELECT id, slug, name FROM onto_base_layers')
        .all() as {
        id: number;
        slug: string;
        name: string;
    ...
    for (const layer of allLayers)
        layerNameMap.set(layer.id, layer.name || layer.slug);

    log(
        'base_resolve',
        'info',
        `Resolving ${terms.length} terms against ${activeItems.length} active base items`,
        `${activeLayerIds.size} active layers`,
        { termCount: terms.length, baseItemCount: activeItems.length }
    );
    ...
    const perLayerStats = new Map<
        string,
        { matches: number; extensions: number }
    >();
    ...
    for (const term of terms) {
        ...
        const allActiveMatches = activeItems.filter(
            item =>
                item.local_name &&
                item.local_name.toLowerCase() === term.name.toLowerCase()
        );

        // Check for ambiguous matches (multiple layers claim the same term)
        if (allActiveMatches.length > 1) {
            const candidates = allActiveMatches.map(
                m => `${layerSlugMap.get(m.layer_id) || m.layer_id}:${m.uri}`
            );
            ambiguousMatches.push({ term: term.name, candidates });
            log(
                'base_resolve',
                'warn',
                `Ambiguous match: "${term.name}" found in ${allActiveMatches.length} layers`,
                candidates.join(', '),
                {
                    term: term.name,
                    candidateCount: allActiveMatches.length,
                    candidates
                }
            );
        }

        let match: BaseLayerItem | null = allActiveMatches[0] ?? null;

        // Also try matching by URI if the term somehow has one in metadata
        if (!match && (term as any).uri) {
            match =
                activeItems.find(item => item.uri === (term as any).uri) ||
                null;
        }
        ...
        if (match) {
            resolvedTerms.push({
                ...term,
                base_item_uri: match.uri,
                layer_id: match.layer_id
            });
            exactMatches++;
            const slug =
                layerSlugMap.get(match.layer_id) || String(match.layer_id);
            matchedItems.push({
                term: term.name,
                layer_slug: slug,
                base_uri: match.uri
            });
            const stats = perLayerStats.get(slug) || {
                matches: 0,
                extensions: 0
            };
            stats.matches++;
            ...
            log(
                'base_resolve',
                'info',
                `Exact match: "${term.name}" → ${match.uri}`,
                `Layer ${slug}`,
                {
                    term: term.name,
                    uri: match.uri,
        ...
        if (globalMatch && !activeLayerIds.has(globalMatch.layer_id)) {
            ...
            autoActivatedLayers++;
            const slug =
                layerSlugMap.get(globalMatch.layer_id) ||
                String(globalMatch.layer_id);
            autoActivatedLayerSlugs.push(slug);
            const layerName = layerNameMap.get(globalMatch.layer_id) || slug;
            log(
                'base_resolve',
                'info',
                `Auto-activated ${layerName} — detected matching entity "${term.name}"`,
                `Matched ${globalMatch.uri}`,
                {
                    term: term.name,
                    layer_id: globalMatch.layer_id,
                    layer_slug: slug
                }
            );
            matchedItems.push({
    ...
    if (
        ...
        activeItems.filter(i => i.item_type === 'class').length > 0
    ) {
...
Respond with JSON: { "specializations": [{ "term": "ExtractedTermName", "parent_base_class": "BaseClassName", "parent_uri": "base:uri" }] }`,
                ...
                const specMap = new Map<
                    string,
                    { parent_base_class: string; parent_uri: string }
                >();
                for (const s of parsed.specializations || []) {
                    specMap.set(s.term.toLowerCase(), {
                        parent_base_class: s.parent_base_class,
                        parent_uri: s.parent_uri
                    });
                }

                const extensionsCreated: {
                    child: string;
                    parent_uri: string;
                }[] = [];
                for (const term of unmatchedTerms) {
                    ...
                    if (spec) {
                        ...
                        const parentItem =
                            activeItems.find(i => i.uri === spec.parent_uri) ||
                            activeItems.find(
                                i =>
                                    (i.local_name || '').toLowerCase() ===
                                    spec.parent_base_class.toLowerCase()
                            );
                        resolvedTerms.push({
                            ...term,
                        ...
                        if (parentItem) {
                            const slug =
                                layerSlugMap.get(parentItem.layer_id) ||
                                String(parentItem.layer_id);
                            const stats = perLayerStats.get(slug) || {
                                matches: 0,
                                extensions: 0
                            };
                        ...
                        log(
                            'base_resolve',
                            'info',
                            `Specialization: "${term.name}" subClassOf "${spec.parent_base_class}"`,
                            spec.parent_uri,
                            { term: term.name, parent: spec.parent_base_class }
                        );
                    } else {
                ...
                log(
                    'base_resolve',
                    'warn',
                    'LLM specialization check failed — passing terms through unresolved'
                );
            }
        } else {
    ...
    for (const [slug, stats] of perLayerStats) {
        ...
        log(
            'base_resolve',
            'info',
            `${layerName}: ${stats.matches} matches, ${stats.extensions} extensions`,
            undefined,
            {
                layer_slug: slug,
    ...
    const extensionsFromSpecs = resolvedTerms
        .filter(
            t =>
                t.parent_base_class &&
                !matchedItems.some(m => m.term === t.name)
        )
        .map(t => ({
            child: t.name,
            parent_uri: t.base_item_uri || t.parent_base_class!
        }));
    ...
    if (ambiguousMatches.length > 0) {
        log(
            'base_resolve',
            'warn',
            `${ambiguousMatches.length} terms had ambiguous matches across multiple layers`,
            ambiguousMatches
                .map(a => `"${a.term}" → ${a.candidates.join(' | ')}`)
                .join('\n'),
...
Respond with JSON: { "terms": [{ "name": "...", "description": "...", "type": "class"|"individual", "confidence": 0.0-1.0 }] }`,
        ...
        try {
            ...
            log(
                'terms',
                'success',
                `Chunk ${i + 1}: found ${chunkTerms.length} terms`,
            ...
            log(
                'terms',
                'warn',
                `Chunk ${i + 1}: failed to parse LLM response`,
                result.slice(0, 200)
            );
        }
    }
...
async function classifyTerms(
    ...
    const termNames = terms
        .map(t => `${t.name} (${t.type}, confidence: ${t.confidence})`)
        .join('\n');
...
async function validateOntology(
    ...
    suggestions: ExtractedTerm[];
    fixedRelations: ExtractedRelation[];
}> {
  ...
  "fixed_relations": [{ "source": "...", "target": "...", "relation_type": "...", "relation_name": "...", "description": "...", "confidence": 0.0-1.0, "fix_description": "what was fixed" }]
    ...
    try {
        ...
        return {
            ...
            suggestions: (parsed.missing_concepts || []).map((t: any) => ({
                ...t,
                source_chunk: -1
            })),
  ...
  `);
    ...
    const tx = db.transaction(() => {
        ...
        for (let i = 0; i < terms.length; i++) {
            const t = terms[i];
            const existing = findNode.get(projectId, t.name) as
                | { id: number }
                | undefined;
            if (existing) {
                ...
                if (t.base_item_uri || t.layer_id) {
                    db.prepare(
                        'UPDATE onto_nodes SET layer_id = COALESCE(?, layer_id), base_item_uri = COALESCE(?, base_item_uri), uri = COALESCE(?, uri) WHERE id = ?'
                    ).run(
                        t.layer_id ?? null,
                        t.base_item_uri ?? null,
                        t.base_item_uri ?? null,
                        existing.id
                    );
                }
            ...
            const posY = isClass
                ? Math.floor(idx / cols) * 180
                : (Math.ceil(classes.length / cols) + 1) * 180 +
                  Math.floor(idx / cols) * 150;
            ...
            if (t.parent_base_class) {
                let parentNodeId = nodeIdMap.get(
                    t.parent_base_class.toLowerCase()
                );
                if (!parentNodeId) {
                    const existingParent = findNode.get(
                        projectId,
                        t.parent_base_class
                ...
                db.prepare(
                    'UPDATE onto_nodes SET parent_id = ? WHERE id = ?'
                ).run(parentNodeId, newNodeId);
                insertEdge.run(
                    projectId,
                    'is_a',
                    'subClassOf',
                    ...
                    parentNodeId,
                    null,
                    `${t.name} IS-A ${t.parent_base_class} (base layer)`,
                    0.9,
                    documentId,
                    jobId
                );
 ...
 */
export function deduplicateExistingNodes(
    db: Database.Database,
    projectId: number
): number {
    const allNodes = db
...
export async function runExtractionPipeline(
    ...
    if (job.document_id) {
        ...
        documents = db
            .prepare('SELECT * FROM onto_documents WHERE project_id = ?')
    ...
    log(
        ...
        `${documents.length} document(s), ${totalWords.toLocaleString()} words, domain: "${domainHint}"`,
        {
    ...
    try {
        ...
        updateJob(db, jobId, {
            pipeline_stage: 'chunk',
            current_step: 'Splitting documents into semantic chunks...',
            progress_pct: 5
        });
        ...
        for (const doc of documents) {
            ...
            db.prepare(
                'UPDATE onto_documents SET chunk_count = ?, status = ? WHERE id = ?'
        ...
        log(
            ...
            `Average ~${Math.round(totalWords / chunks.length)} words per chunk`,
        ...
        if (baseLayerItems.length > 0) {
            log(
                'pipeline',
                'info',
                `Base vocabulary loaded: ${baseLayerItems.length} items for LLM context`
            );
        }

        // ── Stage 2: EXTRACT TERMS ───────────────────────────────────────────────
        updateJob(db, jobId, {
            pipeline_stage: 'terms',
            current_step: `Extracting terms from ${chunks.length} chunks...`,
            progress_pct: 10
        });
        log(
            'terms',
            ...
            `${chunks.length} chunks to process with AI`
        );

        const makeCtx = (stage: string) => ({ log, db, jobId, stage });

        let terms = await extractTerms(
            chunks,
            domainHint,
            existingTerms,
            log,
        ...
        log(
            'terms',
            'success',
            `Extracted ${terms.length} raw terms`,
            `${classes.length} classes, ${individuals.length} individuals`,
            {
                total: terms.length,
        ...
        updateJob(db, jobId, {
            ...
            current_step: `Refining classification of ${terms.length} terms...`,
            progress_pct: 30
        });
        log(
            'classify',
            'milestone',
            'Refining term classifications',
            `Sending ${terms.length} terms for AI review`
        );

        const termsBefore = terms.length;
        terms = await classifyTerms(
            terms,
            domainHint,
            existingTerms,
            makeCtx('classify'),
        ...
        log(
            'classify',
            'success',
            `Refined to ${terms.length} terms`,
            merged > 0 ? `Merged ${merged} duplicates` : 'No duplicates found',
            {
                finalCount: terms.length,
                merged,
                reclassified
            }
        );
        ...
        updateJob(db, jobId, {
            pipeline_stage: 'base_resolve',
            current_step: `Resolving ${terms.length} terms against base layer vocabulary...`,
            progress_pct: 38
        });
        log(
            'base_resolve',
            'milestone',
            'Resolving terms against base layer vocabulary',
            `${terms.length} terms to check`
        );

        terms = await resolveBaseLayerItems(
            db,
            job.project_id,
            terms,
            log,
            makeCtx('base_resolve')
        );
        ...
        updateJob(db, jobId, {
            ...
            current_step: `Building taxonomy from ${classesForTax.length} classes...`,
            progress_pct: 45
        });
        log(
            'taxonomy',
            ...
            `${classesForTax.length} classes to organize`
        );
        ...
        const rootClasses =
            classesForTax.length - new Set(taxonomy.map(t => t.child)).size;
        log(
            'taxonomy',
            'success',
            `Built ${taxonomy.length} IS-A relationships`,
            `${rootClasses} root classes, max depth estimable from hierarchy`,
            {
                relationships: taxonomy.length,
                rootClasses
            }
        );
        ...
        // ── Stage 6: RELATIONS ───────────────────────────────────────────────────
        updateJob(db, jobId, {
            pipeline_stage: 'relations',
            current_step: 'Extracting relationships...',
            progress_pct: 60
        });
        log(
            'relations',
            'milestone',
            'Extracting non-taxonomic relationships',
            `Between ${terms.length} terms`
        );

        let relations = await extractRelations(
            terms,
            chunks,
            domainHint,
            makeCtx('relations')
        ...
        const objProps = relations.filter(
            r => r.relation_type === 'object_property'
        ).length;
        const dataProps = relations.filter(
            r => r.relation_type === 'data_property'
        ).length;
        log(
            'relations',
            'success',
            `Found ${relations.length} relationships`,
            `${objProps} object properties, ${dataProps} data properties`,
            {
        ...
        updateJob(db, jobId, {
            ...
            current_step: 'Running metacognitive validation...',
            progress_pct: 75
        });
        log(
            'validate',
            'milestone',
            'Running Ontogenia metacognitive validation',
            'Checking interpretation, reflection, evaluation, and testing'
        );
        ...
        if (validation.issues.length > 0) {
            ...
            log(
                ...
                validation.issues
                    .map(i => `[${i.severity}] ${i.entity}: ${i.description}`)
                    .join('\n'),
        ...
        if (validation.suggestions.length > 0) {
            ...
            log(
                'validate',
                'info',
                `Added ${validation.suggestions.length} suggested concepts`,
        ...
        if (validation.fixedRelations.length > 0) {
            ...
            log(
                'validate',
                'info',
                `Applied ${validation.fixedRelations.length} relation fixes`,
                validation.fixedRelations
                    .map(
                        (r: any) =>
                            r.fix_description || `${r.source} → ${r.target}`
                    )
                    .join(', ')
            );
        }
        ...
        // keeping the entry with the highest confidence (and preserving base layer annotations).
        {
            ...
            if (beforeDedup > terms.length) {
                log(
                    'merge',
                    'info',
                    `Pre-merge dedup: ${beforeDedup} → ${terms.length} terms`,
        ...
        updateJob(db, jobId, {
            ...
            current_step: 'Merging into knowledge graph...',
            progress_pct: 90
        });
        log(
            'merge',
            ...
            `${terms.length} terms + ${taxonomy.length} taxonomy + ${relations.length} relations`
        );
        ...
        log(
            'merge',
            'success',
            `Created ${nodesCreated} nodes and ${edgesCreated} edges`,
            `${terms.length - nodesCreated} terms matched existing nodes (deduplication)`,
            {
                nodesCreated,
                edgesCreated,
                deduplicated: terms.length - nodesCreated
            }
        );
