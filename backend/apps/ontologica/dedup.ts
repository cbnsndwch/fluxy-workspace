...
async function getExtractor() {
  if (!extractorPromise) {
    extractorPromise = (async () => {
      ...
      console.log('[dedup] Loading embedding model (first time may download ~80MB)...');
...
async function findDuplicateClusters(
  ...
  const dismissedSet = new Set(
    dismissals.map(d => `${Math.min(d.node_a_id, d.node_b_id)}-${Math.max(d.node_a_id, d.node_b_id)}`)
  );
  ...
  for (let pct = 99; pct >= 50; pct--) {
    ...
    for (const count of roots.values()) {
      if (count >= 2) { available_thresholds.push(pct / 100); break; }
    }
  }
...
export function registerDedupRoutes(r: Router, db: Database.Database): void {
  // Scan for duplicates
  r.get('/api/ontologica/projects/:projectId/duplicates', async (req: Request, res: Response) => {
    try {
      ...
      const result = await findDuplicateClusters(db, projectId, Math.max(0.5, Math.min(0.99, threshold)));
      res.json({ clusters: result.clusters, available_thresholds: result.available_thresholds, threshold, computed_at: new Date().toISOString() });
    } catch (err: any) {
      console.error('[dedup] Scan error:', err);
      res.status(500).json({ error: err.message || 'Deduplication scan failed' });
    }
  });

  // Merge duplicates (also accepts dismissals to persist "not duplicates" decisions)
  r.post('/api/ontologica/projects/:projectId/merge', (req: Request, res: Response) => {
    try {
      const projectId = Number(req.params.projectId);
      const project = db.prepare('SELECT id FROM onto_projects WHERE id = ?').get(projectId);
      if (!project) return res.status(404).json({ error: 'Project not found' });
      ...
      const { merges, dismissals } = req.body as {
        merges?: Array<{ canonical_id: number; duplicate_ids: number[] }>;
        ...
        const saveTx = db.transaction(() => {
          for (const group of dismissals) {
            for (let i = 0; i < group.node_ids.length; i++) {
              for (let j = i + 1; j < group.node_ids.length; j++) {
                const a = Math.min(group.node_ids[i], group.node_ids[j]);
                const b = Math.max(group.node_ids[i], group.node_ids[j]);
      ...
      if (!merges?.length && !dismissals?.length) {
        return res.status(400).json({ error: 'No merges or dismissals provided' });
      }

      // Validate all merge node IDs belong to the project
      let totalEdges = 0, totalNodes = 0;
      if (merges?.length) {
        const allIds = merges.flatMap(m => [m.canonical_id, ...m.duplicate_ids]);
        const existing = new Set(
          (db.prepare(`SELECT id FROM onto_nodes WHERE project_id = ? AND id IN (${allIds.map(() => '?').join(',')})`)
            .all(projectId, ...allIds) as any[]).map(r => r.id)
        );
        const missing = allIds.filter(id => !existing.has(id));
        if (missing.length) return res.status(400).json({ error: `Node IDs not found: ${missing.join(', ')}` });

        for (const m of merges) {
          const result = mergeNodes(db, projectId, m.canonical_id, m.duplicate_ids);
          totalEdges += result.edges_redirected;
          totalNodes += result.nodes_deleted;
        }
      }

      res.json({ ok: true, total_edges_redirected: totalEdges, total_nodes_deleted: totalNodes, dismissals_saved: dismissalsSaved });
    } catch (err: any) {
  ...
  r.delete('/api/ontologica/projects/:projectId/dismissals', (req: Request, res: Response) => {
    try {
      const projectId = Number(req.params.projectId);
      const result = db.prepare('DELETE FROM onto_dedup_dismissals WHERE project_id = ?').run(projectId);
