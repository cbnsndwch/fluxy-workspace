...
IMPORTANT: Respond with ONLY valid JSON. No markdown, no explanation, just the JSON object.`;
    ...
    try {
        ...
        if (!response.message)
            response.message =
                'I understood your input but had trouble formatting my response.';
        ...
        try {
            switch (action.type) {
                ...
                case 'add_individual': {
                    ...
                    const nodeType =
                        action.type === 'add_class' ? 'class' : 'individual';
                    let parentId: number | null = null;
                    if (action.parent) {
                        const parentNode = findNode.get(
                            projectId,
                            action.parent
                        ) as any;
                    ...
                    if (parentId) {
                        insertEdge.run(
                            ...
                            parentId,
                            null,
                            `${action.name} IS-A ${action.parent}`,
                            0.9
                        );
                    }
                ...
                case 'add_object_property': {
                    const sourceNode = findNode.get(
                        projectId,
                        action.domain || ''
                    ) as any;
                    const targetNode = findNode.get(
                        projectId,
                        action.range || ''
                    ) as any;
                    if (!sourceNode || !targetNode) {
                        results.push({
                            ...action,
                            success: false,
                            error: `Missing node: ${!sourceNode ? action.domain : action.range}`
                        });
                        break;
                    }
                    ...
                    results.push({
                        ...action,
                        success: true,
                        created_id: Number(result.lastInsertRowid)
                    });
                    break;
                }

                case 'add_data_property': {
                    const domainNode = findNode.get(
                        projectId,
                        action.domain || ''
                    ) as any;
                    if (!domainNode) {
                        results.push({
                            ...action,
                            success: false,
                            error: `Missing node: ${action.domain}`
                        });
                        break;
                    }
                    const result = insertEdge.run(
                ...
                case 'add_is_a': {
                    ...
                    if (!childNode || !parentNode) {
                        results.push({
                            ...action,
                            success: false,
                            error: `Missing node: ${!childNode ? action.name : action.parent}`
                        });
                        break;
                    }
                    ...
                    ).run(parentNode.id, childNode.id);
                    const result = insertEdge.run(
                        projectId,
                        'is_a',
                        'subClassOf',
                ...
                case 'remove_class': {
                    ...
                    if (node) {
                        db.prepare(
                            'UPDATE onto_nodes SET status = ? WHERE id = ?'
                ...
                case 'update_class': {
                    ...
                    if (node) {
                        if (action.description) {
                            db.prepare(
                                'UPDATE onto_nodes SET description = ? WHERE id = ?'
                            ).run(action.description, node.id);
                        }
                        results.push({
                            ...action,
                            success: true,
