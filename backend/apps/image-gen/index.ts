...
export function createRouter(
    ...
    router.get('/api/images', (_req, res) => {
        ...
        const files = fs
            ...
            .filter(
                f =>
                    /\.(png|jpg|jpeg|webp|gif)$/i.test(f) && /^[\w.-]+$/.test(f)
            )
            .map(filename => {
    ...
    router.get('/api/image-gen/history', (_req, res) => {
        const rows = db
            .prepare(
                `SELECT * FROM image_generations ORDER BY created_at DESC LIMIT 100`
            )
            .all();
        res.json(rows);
    });
    ...
    router.post('/api/image-gen/generate', async (req, res) => {
        const { prompt, model, size, quality, style, imageBase64 } =
            req.body as {
                prompt: string;
        ...
        try {
            ...
            if (model === 'imagen-4') {
                ...
                if (imageBase64) {
                    ...
                    const rawB64 = imageBase64.includes(',')
                        ? imageBase64.split(',')[1]
                        : imageBase64;
                    ...
                    b64 = response.generatedImages![0].image!
                ...
                if (imageBase64) {
                    ...
                    try {
                        ...
                        if (!editB64)
                            throw new Error(
                                'No image data returned from DALL-E edit'
                            );
                        b64 = editB64;
                    } finally {
                    ...
                    const imgSize = validSizes.includes(size)
                        ? size
                        : '1024x1024';
                    ...
                    if (!genB64)
                        throw new Error(
                            'No image data returned from DALL-E generate'
                        );
                    b64 = genB64;
                }
            }
    ...
    router.delete('/api/image-gen/:id', (req, res) => {
        ...
        db.prepare(`DELETE FROM image_generations WHERE id=?`).run(
            req.params.id
        );
        res.json({ ok: true });
    });
