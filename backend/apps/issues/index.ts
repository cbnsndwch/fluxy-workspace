...
export function createRouter(db: InstanceType<typeof Database>, WORKSPACE: string) {
  ...
  router.post("/api/issues", (req, res) => {
    ...
    res
      .status(201)
      .json(db.prepare(`SELECT * FROM workspace_issues WHERE id=?`).get(r.lastInsertRowid));
  });
  ...
  router.post("/api/issues/dispatch", (req, res) => {
    ...
    const batchResult = db
      .prepare(`INSERT INTO dispatch_batches (issue_ids, status) VALUES (?, 'working')`)
    ...
    for (const id of issueIds) {
      ...
      const issue = db.prepare(`SELECT * FROM workspace_issues WHERE id=?`).get(id) as Record<
        string,
        unknown
      >;
      if (!issue) continue;
  ...
  router.patch("/api/issues/:id", (req, res) => {
    ...
    db.prepare(`UPDATE workspace_issues SET ${sets}, updated_at=datetime('now') WHERE id=?`).run(
      ...vals,
      req.params.id,
    );
  ...
  router.patch("/api/issues/:id/agent-update", (req, res) => {
    ...
    db.prepare(`UPDATE workspace_issues SET ${sets.join(", ")} WHERE id=?`).run(
      ...vals,
      req.params.id,
    );
    const issue = db.prepare(`SELECT * FROM workspace_issues WHERE id=?`).get(req.params.id) as
  ...
  router.get("/api/dispatch-batches", (_req, res) => {
    const batches = db
      .prepare(`SELECT * FROM dispatch_batches ORDER BY created_at DESC LIMIT 20`)
  ...
  router.get("/api/dispatch-batches/:id", (req, res) => {
    const batch = db.prepare(`SELECT * FROM dispatch_batches WHERE id=?`).get(req.params.id) as
      | Record<string, unknown>
      | undefined;
