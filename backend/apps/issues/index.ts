...
const APP_FILE_PATHS: Record<string, string[]> = {
    ...
    'image-studio': [
        'client/src/components/ImageGen/',
        'backend/apps/image-gen/'
    ],
    'marble-studio': [
        'client/src/components/MarbleStudio/',
        'backend/apps/marble-studio/'
    ],
    ...
    'flow-capture': [
        'client/src/components/FlowCapture/',
        'backend/apps/flow-capture/'
    ],
...
function buildWorkerTask(
    ...
    const description =
        (issue.description as string | null) ?? '(no description)';
...
function buildSyncTask(
    ...
    const issueList = issues
        .map(i => `- #${i.id}: ${i.title} → branch \`issue/${i.id}\``)
        .join('\n');
    //   const branchList = issues.map((i) => `issue/${i.id}`).join(" ");
    const worktreeCleanup = issues
        .map(
            i =>
...
function readCrons(WORKSPACE: string): unknown[] {
    ...
    try {
        return JSON.parse(fs.readFileSync(cronsPath, 'utf-8'));
    } catch {
        return [];
    }
}
...
function addIssueCron(WORKSPACE: string, issueId: number): void {
    ...
    const filtered = (crons as Array<Record<string, unknown>>).filter(
        c => c.id !== id
    );
    filtered.push({
        id,
...
export function createRouter(
    ...
    router.get('/api/issues', (req, res) => {
        const { status, priority, category, q } = req.query as Record<
            string,
            string
        >;
    ...
    router.post('/api/issues', (req, res) => {
        ...
        const r = db
            .prepare(
                `INSERT INTO workspace_issues (title,description,status,priority,category,app,attachments) VALUES (?,?,?,?,?,?,?)`
            )
            .run(
                title,
                description || null,
        ...
        res.status(201).json(
            db
                .prepare(`SELECT * FROM workspace_issues WHERE id=?`)
    ...
    router.post('/api/issues/dispatch', (req, res) => {
        ...
        if (!issueIds?.length)
            return res.status(400).json({ error: 'issueIds required' });
        ...
        fs.mkdirSync(path.join(WORKSPACE, '.claude', 'worktrees'), {
            recursive: true
        });
        ...
        for (const id of issueIds) {
            ...
            db.prepare(
                `UPDATE workspace_issues SET status='in-progress', agent_status='queued', agent_branch=?, batch_id=?, dispatched_at=datetime('now'), updated_at=datetime('now') WHERE id=?`
            ...
            fs.writeFileSync(
                path.join(TASKS_DIR, `issue-worker-${id}.md`),
                taskContent
            );
    ...
    router.patch('/api/issues/:id', (req, res) => {
        ...
        const vals = fields.map(f =>
            f === 'attachments' ? JSON.stringify(req.body[f]) : req.body[f]
        );
        db.prepare(
            `UPDATE workspace_issues SET ${sets}, updated_at=datetime('now') WHERE id=?`
    ...
    router.patch('/api/issues/:id/agent-update', (req, res) => {
        ...
        if (agent_status !== undefined) {
            sets.push('agent_status=?');
            vals.push(agent_status);
        }
        if (agent_log !== undefined) {
            sets.push('agent_log=?');
            vals.push(agent_log);
        }
        if (agent_branch !== undefined) {
            sets.push('agent_branch=?');
            vals.push(agent_branch);
        }

        db.prepare(
            `UPDATE workspace_issues SET ${sets.join(', ')} WHERE id=?`
        ).run(...vals, req.params.id);
        const issue = db
            .prepare(`SELECT * FROM workspace_issues WHERE id=?`)
        ...
        if (
            issue?.batch_id &&
            (agent_status === 'done' || agent_status === 'failed')
        ) {
            const batchId = issue.batch_id as number;
            const batchIssues = db
                .prepare(`SELECT * FROM workspace_issues WHERE batch_id=?`)
                .all(batchId) as Array<Record<string, unknown>>;

            const allDone = batchIssues.every(
                i => i.agent_status === 'done' || i.agent_status === 'failed'
            );

            if (allDone) {
                // Create sync task and CRON
                const syncContent = buildSyncTask(
                    batchId,
                    batchIssues,
                    WORKSPACE
                );
                fs.writeFileSync(
                    path.join(TASKS_DIR, `issue-sync-${batchId}.md`),
                    syncContent
                );
                addSyncCron(WORKSPACE, batchId);

                db.prepare(
                    `UPDATE dispatch_batches SET status='syncing', updated_at=datetime('now') WHERE id=?`
                ).run(batchId);
            }
        }
    ...
    router.put('/api/issues/:id', (req, res) => {
        ...
        db.prepare(
            `UPDATE workspace_issues SET title=?,description=?,status=?,priority=?,category=?,app=?,attachments=?,updated_at=datetime('now') WHERE id=?`
        ).run(
            title,
            description || null,
            status || 'open',
    ...
    router.delete('/api/issues/:id', (req, res) => {
        const issue = db
            .prepare(`SELECT attachments FROM workspace_issues WHERE id=?`)
            .get(req.params.id) as { attachments: string } | undefined;
        if (issue) {
            try {
        ...
        db.prepare(`DELETE FROM workspace_issues WHERE id=?`).run(
            req.params.id
        );
        res.json({ ok: true });
    });
    ...
    router.get('/api/dispatch-batches', (_req, res) => {
        ...
        const enriched = batches.map(batch => {
            ...
            const issues = issueIds.length
                ? db
                      .prepare(
                          `SELECT id, title, agent_status, agent_log, agent_branch FROM workspace_issues WHERE id IN (${issueIds.map(() => '?').join(',')})`
                      )
                      .all(...issueIds)
                : [];
            return { ...batch, issues };
    ...
    router.get('/api/dispatch-batches/:id', (req, res) => {
        ...
        const issues = issueIds.length
            ? db
                  .prepare(
                      `SELECT id, title, status, agent_status, agent_log, agent_branch FROM workspace_issues WHERE id IN (${issueIds.map(() => '?').join(',')})`
                  )
                  .all(...issueIds)
            : [];
    ...
    router.post('/api/dispatch-batches/:id/sync-complete', (req, res) => {
        const { status, sync_report } = req.body as {
            status: string;
            sync_report: string;
        };
        db.prepare(
            `UPDATE dispatch_batches SET status=?, sync_report=?, updated_at=datetime('now') WHERE id=?`
