...
export function createRouter(db: InstanceType<typeof Database>) {
    ...
    router.get('/api/db/tables', (_req, res) => {
        ...
        const result = tables.map(({ name }) => {
            ...
            const { n } = db.prepare(`SELECT COUNT(*) as n FROM ${JSON.stringify(name)}`).get() as { n: number };
    ...
    router.get('/api/db/:table/rows', (req, res) => {
        ...
        const { n: total } = db.prepare(`SELECT COUNT(*) as n FROM ${JSON.stringify(table)}`).get() as { n: number };
    ...
    router.post('/api/db/:table/rows', (req, res) => {
        ...
        if (cols.length === 0) return res.status(400).json({ error: 'No columns provided' });

        const placeholders = cols.map(() => '?').join(',');
        const colList = cols.map(c => JSON.stringify(c)).join(',');
        try {
            const r = db.prepare(`INSERT INTO ${JSON.stringify(table)} (${colList}) VALUES (${placeholders})`).run(...cols.map(c => body[c] === '' ? null : body[c]));
            res.status(201).json(db.prepare(`SELECT * FROM ${JSON.stringify(table)} WHERE rowid=?`).get(r.lastInsertRowid));
        } catch (e: unknown) {
    ...
    router.put('/api/db/:table/rows/:id', (req, res) => {
        ...
        const colInfo = db.prepare(`PRAGMA table_info(${JSON.stringify(table)})`).all() as { name: string }[];
        const hasUpdated = colInfo.some(c => c.name === 'updated_at');
        const finalSet = hasUpdated ? `${setClause},updated_at=datetime('now')` : setClause;
        try {
            db.prepare(`UPDATE ${JSON.stringify(table)} SET ${finalSet} WHERE id=?`).run(...cols.map(c => body[c] === '' ? null : body[c]), id);
            res.json(db.prepare(`SELECT * FROM ${JSON.stringify(table)} WHERE id=?`).get(id));
        } catch (e: unknown) {
            res.status(400).json({ error: e instanceof Error ? e.message : String(e) });
        }
    });
    ...
    router.delete('/api/db/:table/rows/:id', (req, res) => {
        ...
        if (!exists) return res.status(404).json({ error: 'Table not found' });
        try {
            db.prepare(`DELETE FROM ${JSON.stringify(table)} WHERE id=?`).run(id);
            res.json({ ok: true });
        } catch (e: unknown) {
            res.status(400).json({ error: e instanceof Error ? e.message : String(e) });
        }
