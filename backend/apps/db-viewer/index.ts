...
export function createRouter(db: InstanceType<typeof Database>) {
  ...
  router.get("/api/db/tables", (_req, res) => {
    ...
    const result = tables.map(({ name }) => {
      ...
      const { n } = db.prepare(`SELECT COUNT(*) as n FROM ${JSON.stringify(name)}`).get() as {
        n: number;
      };
  ...
  router.get("/api/db/:table/rows", (req, res) => {
    ...
    const rows = db
      ...
      .all(limit, offset);
    const { n: total } = db.prepare(`SELECT COUNT(*) as n FROM ${JSON.stringify(table)}`).get() as {
      n: number;
    };
  ...
  router.post("/api/db/:table/rows", (req, res) => {
    ...
    try {
      const r = db
        .prepare(`INSERT INTO ${JSON.stringify(table)} (${colList}) VALUES (${placeholders})`)
        .run(...cols.map((c) => (body[c] === "" ? null : body[c])));
      res
        .status(201)
        .json(
          db.prepare(`SELECT * FROM ${JSON.stringify(table)} WHERE rowid=?`).get(r.lastInsertRowid),
        );
    } catch (e: unknown) {
  ...
  router.put("/api/db/:table/rows/:id", (req, res) => {
    ...
    const colInfo = db.prepare(`PRAGMA table_info(${JSON.stringify(table)})`).all() as {
      name: string;
    }[];
    const hasUpdated = colInfo.some((c) => c.name === "updated_at");
    const finalSet = hasUpdated ? `${setClause},updated_at=datetime('now')` : setClause;
    try {
      db.prepare(`UPDATE ${JSON.stringify(table)} SET ${finalSet} WHERE id=?`).run(
