...
function getConn(workspace: string): Promise<DuckDBConnection> {
    if (!_connPromise) {
        _connPromise = DuckDBInstance.create(
            path.join(workspace, 'analytics.duckdb')
        )
...
export function createRouter(workspace: string) {
    ...
    router.get('/api/analytics/overview', async (_req, res) => {
        try {
            ...
            const [
                ...
                conn.runAndReadAll(
                    `SELECT COUNT(DISTINCT session_id) AS n FROM analytics_events WHERE session_id IS NOT NULL`
                ),
                conn.runAndReadAll(
                    `SELECT COUNT(*) AS n FROM analytics_events WHERE created_at >= current_date`
                ),
                conn.runAndReadAll(
                    `SELECT COUNT(*) AS n FROM analytics_events WHERE created_at >= now() - INTERVAL '7 days'`
                ),
                conn.runAndReadAll(`
            ...
            res.json({
                ...
                uniqueSessions: Number(
                    (sessRow.getRowObjects()[0] as any)?.n ?? 0
                ),
    ...
    router.get('/api/analytics/apps/:appId', async (req, res) => {
        try {
            ...
            const daysInt = Math.min(
                Math.max(
                    parseInt((req.query.days as string) || '7', 10) || 7,
                    1
                ),
                90
            );
