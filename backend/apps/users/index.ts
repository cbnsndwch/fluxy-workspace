...
export const APP_PERMISSIONS: Record<
    string,
    { action: string; label: string }[]
> = {
    chat: [{ action: 'access', label: 'Access Fluxy chat' }],
    'app-ideas': [
    ...
    'db-viewer': [{ action: 'view', label: 'Browse database tables' }],
    docs: [
...
export function createRouter(db: InstanceType<typeof Database>) {
    ...
    router.put('/api/users/:id/roles', (req, res) => {
        ...
        db.prepare(`DELETE FROM user_roles WHERE user_id = ?`).run(
            req.params.id
        );
    ...
    router.get('/api/roles', (_req, res) => {
        const roles = db
            ...
            .all() as any[];
        const perms = db
            .prepare(`SELECT * FROM role_permissions`)
            .all() as any[];
        ...
        try {
            ...
            if (permissions?.length) {
                const ins = db.prepare(
                    `INSERT OR IGNORE INTO role_permissions (role_id, app, action) VALUES (?, ?, ?)`
                );
            ...
            const perms = db
                .prepare(
                    `SELECT app, action FROM role_permissions WHERE role_id = ?`
                )
                .all(roleId);
    ...
    router.put('/api/roles/:id', (req, res) => {
        ...
        if (role.is_system)
            return res
                .status(403)
                .json({ error: 'Cannot modify system roles' });
        ...
        if (permissions?.length) {
            ...
            for (const p of permissions)
                ins.run(req.params.id, p.app, p.action);
        }
    ...
    router.delete('/api/roles/:id', (req, res) => {
        ...
        if (role.is_system)
            return res
                .status(403)
                .json({ error: 'Cannot delete system roles' });
