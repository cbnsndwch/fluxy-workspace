...
const stmtInsertLore = db.prepare<
  [number, string | null, string | null, string | null, string | null, string | null]
>(`
...
const migrate = db.transaction(() => {
  for (let i = 0; i < documents.length; i++) {
    ...
    try {
      ...
      const artistSlug =
        (doc.artist_slug as string | undefined) ?? toSlug(artist.split(",")[0].trim());
      const trackSlug = (doc.track_slug as string | undefined) ?? toSlug(title);
