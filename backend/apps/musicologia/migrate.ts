...
if (inputFlag === -1 || !args[inputFlag + 1]) {
    console.error(
        'Usage: pnpm dlx tsx migrate.ts --input /path/to/export.json'
    );
    process.exit(1);
}
...
function parseLrc(
    lrc: string
): { time_seconds: number; text: string; line_index: number }[] {
...
const stmtInsertLore = db.prepare<
    [
        number,
        string | null,
        string | null,
    ...
    documents = raw
        ...
        .map((line, i) => {
            try {
                ...
                throw new Error(
                    `Failed to parse line ${i + 1}: ${e instanceof Error ? e.message : String(e)}`
                );
            }
        });
}
...
const migrate = db.transaction(() => {
    for (let i = 0; i < documents.length; i++) {
        ...
        if (i > 0 && i % 100 === 0) {
            console.log(
                `  [${i}/${documents.length}] imported=${imported} skipped=${skipped} errors=${errors}`
            );
        }
        ...
        try {
            ...
            const artistSlug =
                (doc.artist_slug as string | undefined) ??
                toSlug(artist.split(',')[0].trim());
            const trackSlug =
                (doc.track_slug as string | undefined) ?? toSlug(title);

            // ------------------------------------------------------------------
            // Skip check — look up by spotify_id
            // ------------------------------------------------------------------
            if (spotifyId) {
                const existing = stmtFindBySpotifyId.get(spotifyId) as
                    | { id: number }
                    | undefined;
                if (existing) {
                    skipped++;
            ...
            if (dna && typeof dna === 'object') {
                const palette =
                    dna.palette != null ? JSON.stringify(dna.palette) : null;
            ...
            if (lore && typeof lore === 'object') {
                ...
                const trivia =
                    lore.trivia != null ? JSON.stringify(lore.trivia) : null;
                const themes =
                    lore.themes != null ? JSON.stringify(lore.themes) : null;
                const credits =
                    lore.credits != null ? JSON.stringify(lore.credits) : null;
            ...
            errors++;
            const docId =
                (doc._id as Record<string, string> | undefined)?.$oid ??
                String(doc.spotify_id ?? `index ${i}`);
            console.error(
                `  ERROR processing document [${docId}]: ${e instanceof Error ? e.message : String(e)}`
            );
        }
    }
});
