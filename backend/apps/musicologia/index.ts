...
function getStoredToken(db: InstanceType<typeof Database>): SpotifyTokenRow | null {
  return (
    (db.prepare("SELECT * FROM musicologia_spotify_tokens ORDER BY id DESC LIMIT 1").get() as
      | SpotifyTokenRow
      | undefined) ?? null
  );
}
...
async function importSpotifyTrack(
  ...
  const coverUrl =
    sp.album?.images?.find((i) => i.width >= 300)?.url ?? sp.album?.images?.[0]?.url ?? null;
  ...
  const existing = db
    .prepare(`SELECT id FROM tracks WHERE JSON_EXTRACT(source_ids,'$.spotify_id') = ?`)
  ...
  if (mbTags && mbTags.length > 0) {
    ...
    if (!loreExists) {
      db.prepare(`INSERT INTO track_lore (track_id, themes) VALUES (?, ?)`).run(
        trackId,
        JSON.stringify(mbTags.slice(0, 10)),
      );
    }
  }
...
export function createRouter(db: InstanceType<typeof Database>) {
  ...
  router.get("/api/musicologia/auth/spotify/callback", async (req, res) => {
    ...
    if (error || !code)
      return res.redirect("/app/musicologia?spotify_error=" + (error || "missing_code"));
  ...
  router.get("/api/musicologia/search", async (req, res) => {
    ...
    if (!token)
      return res
        .status(401)
        .json({ error: "Spotify not connected. Connect at /musicologia settings." });
    ...
    const results = items.map((t) => ({
      ...
      cover_url:
        t.album?.images?.find((i) => i.width >= 300)?.url ?? t.album?.images?.[0]?.url ?? null,
  ...
  router.get("/api/musicologia/tracks/:artistSlug/:trackSlug", (req, res) => {
    ...
    const track = db
      .prepare(`SELECT * FROM tracks WHERE artist_slug = ? AND track_slug = ?`)
    ...
    const sections = db
      ...
      .get(trackId);
    const lyrics = db
      .prepare(`SELECT * FROM track_lyrics WHERE track_id = ? ORDER BY start_ms`)
      .all(trackId);
    const lrcLyrics = db
      .prepare(
  ...
  router.patch("/api/musicologia/tracks/:id", (req, res) => {
    ...
    const existing = db.prepare(`SELECT * FROM tracks WHERE id = ?`).get(req.params.id) as
  ...
  router.post("/api/musicologia/tracks/:id/generate-lore", async (req, res) => {
    ...
    const track = db.prepare(`SELECT * FROM tracks WHERE id = ?`).get(trackId) as
      | Record<string, unknown>
      | undefined;
    if (!track) return res.status(404).json({ error: "Track not found" });

    const dna = db.prepare(`SELECT * FROM track_dna WHERE track_id = ?`).get(trackId) as Record<
      string,
      unknown
    > | null;
    ...
    if (!loreOutput) {
      return res
        .status(503)
        .json({ error: "Lore generation failed. Claude token may be unavailable." });
    }
  ...
  router.post("/api/musicologia/tracks/:id/generate-palette", async (req, res) => {
    ...
    if (dna) {
      db.prepare(`UPDATE track_dna SET palette=?,updated_at=datetime('now') WHERE track_id=?`).run(
        JSON.stringify(palette),
        trackId,
      );
    } else {
      db.prepare(`INSERT INTO track_dna (track_id, palette) VALUES (?,?)`).run(
        trackId,
        JSON.stringify(palette),
      );
    }
    ...
    for (const trackId of trackIds) {
      ...
      try {
        const dna = db.prepare(`SELECT * FROM track_dna WHERE track_id = ?`).get(trackId) as Record<
          ...
          unknown
        > | null;
        const sourceIds = JSON.parse((track.source_ids as string) || "{}") as Record<
          string,
          string
        >;
        const spotifyId = sourceIds.spotify_id;
  ...
  router.post("/api/musicologia/users/:userId/follow", (req, res) => {
    ...
    const target = db.prepare(`SELECT id, login, name FROM users WHERE id = ?`).get(targetId) as
      | { id: number; login: string; name: string | null }
      | undefined;
    if (!target) return res.status(404).json({ error: "User not found" });

    db.prepare(`INSERT OR IGNORE INTO music_follows (follower_id, following_id) VALUES (?, ?)`).run(
      me.id,
      targetId,
    );
  ...
  router.delete("/api/musicologia/users/:userId/follow", (req, res) => {
    const me = getSessionUser(db, req);
    if (!me) return res.status(401).json({ error: "Not authenticated" });
    db.prepare(`DELETE FROM music_follows WHERE follower_id = ? AND following_id = ?`).run(
      me.id,
  ...
  router.get("/api/musicologia/users/:userId/follow-status", (req, res) => {
    ...
    const row = db
      .prepare(`SELECT 1 FROM music_follows WHERE follower_id = ? AND following_id = ?`)
  ...
  router.get("/api/musicologia/tracks/:id/reactions", (req, res) => {
    ...
    const myReaction = me
      ? ((
          db
            .prepare(`SELECT emoji FROM music_reactions WHERE user_id = ? AND track_id = ?`)
  ...
  router.post("/api/musicologia/tracks/:id/reactions", (req, res) => {
    ...
    const trackId = Number(req.params.id);
    const track = db.prepare(`SELECT id, title, artist FROM tracks WHERE id = ?`).get(trackId) as
    ...
    if (existing?.emoji === emoji) {
      // Toggle off
      db.prepare(`DELETE FROM music_reactions WHERE user_id = ? AND track_id = ?`).run(
        me.id,
        trackId,
      );
  ...
  router.post("/api/musicologia/comments", (req, res) => {
    ...
    if (!parent_id) {
      ...
      if (target_type === "track") {
        const t = db.prepare(`SELECT title, artist FROM tracks WHERE id = ?`).get(target_id) as
  ...
  router.delete("/api/musicologia/comments/:id", (req, res) => {
    ...
    const comment = db.prepare(`SELECT user_id FROM music_comments WHERE id = ?`).get(commentId) as
      | { user_id: number }
      | undefined;
  ...
  router.post("/api/musicologia/users/:userId/follow", (req, res) => {
    ...
    try {
      ...
      const followed = db.prepare(`SELECT login, name FROM users WHERE id = ?`).get(followingId) as
  ...
  router.post("/api/musicologia/tracks/:id/reactions", (req, res) => {
    ...
    const track = db.prepare(`SELECT id, title FROM tracks WHERE id = ?`).get(trackId) as
  ...
  router.post("/api/musicologia/comments", (req, res) => {
    ...
    if (!parent_id) {
      let objectTitle: string | null = null;
      if (target_type === "track") {
        const t = db.prepare(`SELECT title FROM tracks WHERE id = ?`).get(target_id) as
          | { title: string }
          | undefined;
        objectTitle = t?.title ?? null;
      } else if (target_type === "playlist") {
        const p = db.prepare(`SELECT title FROM playlists WHERE id = ?`).get(target_id) as
          | { title: string }
          | undefined;
  ...
  router.delete("/api/musicologia/users/:userId/follow", (req, res) => {
    ...
    db.prepare(`DELETE FROM musicologia_follows WHERE follower_id = ? AND following_id = ?`).run(
      me.id,
      req.params.userId,
    );
    res.json({ ok: true });
  ...
  router.get("/api/musicologia/feed/global", (req, res) => {
    ...
    const offset = parseInt(String(req.query.offset ?? "0"), 10);
    const rows = db
      .prepare(`SELECT * FROM musicologia_activity_feed ORDER BY created_at DESC LIMIT ? OFFSET ?`)
      .all(limit, offset);
    res.json(rows);
  });
  ...
  router.get("/api/musicologia/tracks/:id/reactions", (req, res) => {
    ...
    if (me) {
      const mine = db
        .prepare(`SELECT emoji FROM musicologia_reactions WHERE user_id = ? AND track_id = ?`)
        .get(me.id, trackId) as { emoji: string } | undefined;
  ...
  router.post("/api/musicologia/tracks/:id/reactions", (req, res) => {
    ...
    if (existing?.emoji === emoji) {
      // Toggle off
      db.prepare(`DELETE FROM musicologia_reactions WHERE user_id = ? AND track_id = ?`).run(
        me.id,
        trackId,
      );
    } else {
    ...
    try {
      ...
      for (const item of tracksData.items || []) {
        ...
        try {
          const result = await importSpotifyTrack(db, token, sp.id);
          if (result.isNew) {
            logAudit(null, "track.created", "track", (result.track as { id: number }).id, {
  ...
  router.post("/api/musicologia/admin/sync-audio-features", async (_req, res) => {
    ...
    if (tracks.length === 0)
      return res.json({ synced: 0, message: "All tracks already have audio features" });
    ...
    try {
      ...
      if (!featuresRes.ok)
        return res
          ...
          .json({ error: "Failed to fetch audio features from Spotify" });
  ...
  router.post("/api/musicologia/admin/batch-lore-generate", async (req, res) => {
    ...
    if (all) {
      ...
      tracks = db
        .prepare(`SELECT id, title, artist FROM tracks WHERE id IN (${placeholders})`)
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      try {
        ...
        const sourceIds = JSON.parse(
          (
            db.prepare(`SELECT source_ids FROM tracks WHERE id = ?`).get(track.id) as
              | { source_ids: string }
              | undefined
          )?.source_ids || "{}",
  ...
  router.post("/api/musicologia/scrobble/tick", async (_req, res) => {
    try {
      ...
      if (!scrobbleState.scrobbled && progressMs >= threshold && threshold > 0) {
        ...
        const albumArt =
          item.album.images.find((i) => i.width >= 300)?.url ?? item.album.images[0]?.url ?? null;
        ...
        const existingStaging = db
          .prepare(`SELECT id, status FROM musicologia_staging WHERE spotify_track_id = ?`)
  ...
  router.get("/api/musicologia/plays", (req, res) => {
    ...
    const rows = db
      .prepare(`SELECT * FROM musicologia_plays ORDER BY played_at DESC LIMIT ? OFFSET ?`)
      .all(limit, offset);
    const { total } = db.prepare(`SELECT COUNT(*) as total FROM musicologia_plays`).get() as {
      total: number;
    };
