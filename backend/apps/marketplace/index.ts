...
function initDB(db: Database.Database) {
    ...
    const existing = db
        .prepare(
            "SELECT value FROM marketplace_settings WHERE key = 'workspace_id'"
        )
        .get() as { value: string } | undefined;
    if (!existing) {
        db.prepare(
            "INSERT INTO marketplace_settings (key, value) VALUES ('workspace_id', ?)"
...
function getSetting(db: Database.Database, key: string): unknown {
    const row = db
        .prepare('SELECT value FROM marketplace_settings WHERE key = ?')
...
export function createRouter(db: Database.Database) {
    ...
    router.post('/api/marketplace/checkout', (req: Request, res: Response) => {
        ...
        if (!tier || !Array.isArray(apps) || apps.length === 0) {
            return res
                .status(400)
                .json({ error: 'tier and apps[] are required' });
        }
    ...
    router.get('/api/marketplace/tokens', (_req: Request, res: Response) => {
        const tokens = db
            .prepare(
                'SELECT * FROM marketplace_tokens ORDER BY created_at DESC'
            )
    ...
    /* ── Revoke token ────────────────────────────────────────────────────── */
    router.delete(
        '/api/marketplace/tokens/:id',
        (req: Request, res: Response) => {
            const result = db
                .prepare(
                    'UPDATE marketplace_tokens SET revoked = 1 WHERE id = ?'
                )
                .run(req.params.id);
            if (result.changes === 0)
            ...
            db.prepare(
                "UPDATE marketplace_orders SET status = 'revoked' WHERE token_id = ?"
            ).run(req.params.id);
            res.json({ ok: true });
        }
    );
    ...
    router.get('/api/marketplace/orders', (_req: Request, res: Response) => {
        const orders = db
            ...
            .all() as Array<Record<string, unknown>>;
        res.json(
            orders.map(o => ({ ...o, apps: JSON.parse(o.apps as string) }))
        );
    });
    ...
    router.post('/api/marketplace/redeem', (req: Request, res: Response) => {
        ...
        const record = db
            .prepare('SELECT * FROM marketplace_tokens WHERE id = ?')
        ...
        if (status === 'redeemed')
            return res
                .status(409)
                .json({ error: 'Token has already been redeemed' });
        ...
        db.prepare(
            "UPDATE marketplace_orders SET status = 'redeemed' WHERE token_id = ?"
        ).run(token);
        ...
        (req: Request, res: Response) => {
            ...
            if (token) {
                ...
                if (status !== 'redeemed' && status !== 'active') {
                    return res
                        .status(410)
                        .json({ error: `Token is ${status}` });
                }
                const apps = JSON.parse(record.apps as string) as string[];
                if (!apps.includes(appId)) {
                    return res
                        .status(403)
                        .json({ error: 'App not included in this token' });
                }
            }
            ...
            if (!app)
                return res
                    .status(404)
                    .json({ error: 'App not found in catalog' });
    ...
    router.get('/api/marketplace/settings', (_req: Request, res: Response) => {
        const rows = db
            .prepare('SELECT * FROM marketplace_settings')
            .all() as Array<{
            key: string;
            value: string;
        }>;
    ...
    router.post(
        '/api/marketplace/report-error',
        (req: Request, res: Response) => {
            const enabled = getSetting(db, 'error_tracking_enabled');
            if (!enabled)
                return res
                    .status(403)
                    .json({ error: 'Error tracking is not enabled' });

            const { appId, workspaceId, errorMessage, errorStack, context } =
                req.body as Record<string, unknown>;

            if (!appId || !errorMessage) {
                return res
                    .status(400)
                    .json({ error: 'appId and errorMessage are required' });
            }
        ...
        `).run(
            ...
            const apiUrl = getSetting(db, 'api_reporting_url') as
                | string
                | undefined;
            const apiEnabled = getSetting(db, 'api_reporting_enabled') as
                | boolean
                | undefined;
            if (apiEnabled && apiUrl) {
                fetch(apiUrl, {
                    ...
                    console.error('[marketplace] Error forwarding report:', err)
                );
            }
    ...
    router.post('/api/marketplace/telemetry', (req: Request, res: Response) => {
        ...
        if (!enabled)
            return res.status(403).json({ error: 'Telemetry is not enabled' });

        const { appId, workspaceId, eventType, payload } = req.body as Record<
            string,
            unknown
        >;
        ...
        if (!appId || !eventType) {
            return res
                .status(400)
                .json({ error: 'appId and eventType are required' });
        }
    ...
    /* ── Reports viewer ───────────────────────────────────────────────────── */
    router.get(
        '/api/marketplace/error-reports',
        (_req: Request, res: Response) => {
            const rows = db
                .prepare(
                    'SELECT * FROM marketplace_error_reports ORDER BY reported_at DESC LIMIT 200'
                )
                .all() as Array<Record<string, unknown>>;
            res.json(
                rows.map(r => ({
    ...
    router.get(
        '/api/marketplace/telemetry-events',
        (_req: Request, res: Response) => {
            const rows = db
                .prepare(
                    'SELECT * FROM marketplace_telemetry ORDER BY recorded_at DESC LIMIT 500'
                )
                .all() as Array<Record<string, unknown>>;
            res.json(
                rows.map(r => ({
    ...
    router.get('/api/marketplace/stats', (_req: Request, res: Response) => {
        const totalTokens = (
            db
                .prepare('SELECT COUNT(*) as n FROM marketplace_tokens')
                .get() as { n: number }
        ).n;
        const activeTokens = (
            db
        ...
        const redeemedTokens = (
            db
                .prepare(
                    'SELECT COUNT(*) as n FROM marketplace_tokens WHERE redeemed_at IS NOT NULL'
                )
                .get() as { n: number }
        ).n;
        const errorReports = (
            db
                .prepare('SELECT COUNT(*) as n FROM marketplace_error_reports')
                .get() as { n: number }
        ).n;
        const telemetryEvents = (
            db
                .prepare('SELECT COUNT(*) as n FROM marketplace_telemetry')
                .get() as { n: number }
        ).n;
