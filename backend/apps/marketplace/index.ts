...
export const APP_CATALOG: CatalogApp[] = [
    ...
    },
    {
        ...
        description: 'Speak your user flow and watch AI render it as a live Mermaid diagram. Persistent sessions, voice + text input.',
        version: '1.0.0',
        icon: 'GitBranch',
    ...
    },
    {
        ...
        description: 'Generate fascinating conversation starters from live tech headlines. Features Steven Mode for maximum chaos.',
        version: '1.0.0',
    ...
    },
    {
        ...
        description: 'Track app usage, visualize events, and understand how your workspace is used — all on your hardware.',
        version: '1.0.0',
        icon: 'BarChart2',
...
function initDB(db: Database.Database) {
    ...
    const existing = db.prepare("SELECT value FROM marketplace_settings WHERE key = 'workspace_id'").get() as { value: string } | undefined;
    if (!existing) {
        db.prepare("INSERT INTO marketplace_settings (key, value) VALUES ('workspace_id', ?)")
            .run(JSON.stringify(crypto.randomUUID().slice(0, 12)));
    }
}

/* ─── Helpers ────────────────────────────────────────────────────────────────*/
function getSetting(db: Database.Database, key: string): unknown {
    const row = db.prepare('SELECT value FROM marketplace_settings WHERE key = ?').get(key) as { value: string } | undefined;
    return row ? JSON.parse(row.value) : undefined;
}

function computeTokenStatus(token: { expires_at: string; redeemed_at: string | null; revoked: number }): 'active' | 'expired' | 'redeemed' | 'revoked' {
...
export function createRouter(db: Database.Database) {
    ...
    router.post('/api/marketplace/checkout', (req: Request, res: Response) => {
        const { tier, apps, price, buyerEmail, notes, expiryHours = 48 } = req.body as {
            tier: string;
            apps: string[];
            price: number;
            buyerEmail?: string;
        ...
        db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(orderId, tier, JSON.stringify(apps), price ?? 0, tokenId, buyerEmail ?? null, notes ?? null);
    ...
    router.get('/api/marketplace/tokens', (_req: Request, res: Response) => {
        const tokens = db.prepare('SELECT * FROM marketplace_tokens ORDER BY created_at DESC').all() as Array<Record<string, unknown>>;
        res.json(tokens.map(t => ({
            ...t,
            apps: JSON.parse(t.apps as string),
            status: computeTokenStatus(t as { expires_at: string; redeemed_at: string | null; revoked: number }),
        })));
    });

    /* ── Revoke token ────────────────────────────────────────────────────── */
    router.delete('/api/marketplace/tokens/:id', (req: Request, res: Response) => {
        const result = db.prepare("UPDATE marketplace_tokens SET revoked = 1 WHERE id = ?").run(req.params.id);
        if (result.changes === 0) return res.status(404).json({ error: 'Token not found' });
        // Update order status too
        db.prepare("UPDATE marketplace_orders SET status = 'revoked' WHERE token_id = ?").run(req.params.id);
        res.json({ ok: true });
    });

    /* ── List orders ─────────────────────────────────────────────────────── */
    router.get('/api/marketplace/orders', (_req: Request, res: Response) => {
        const orders = db.prepare('SELECT * FROM marketplace_orders ORDER BY created_at DESC').all() as Array<Record<string, unknown>>;
    ...
    router.post('/api/marketplace/redeem', (req: Request, res: Response) => {
        ...
        const record = db.prepare('SELECT * FROM marketplace_tokens WHERE id = ?').get(token) as Record<string, unknown> | undefined;

        if (!record) return res.status(404).json({ error: 'Token not found' });

        const status = computeTokenStatus(record as { expires_at: string; redeemed_at: string | null; revoked: number });
        ...
        if (status === 'redeemed') return res.status(409).json({ error: 'Token has already been redeemed' });

        // Mark redeemed
        db.prepare("UPDATE marketplace_tokens SET redeemed_at = datetime('now'), redeemed_by = ? WHERE id = ?")
            .run(req.ip, token);
    ...
    router.get('/api/marketplace/settings', (_req: Request, res: Response) => {
        const rows = db.prepare('SELECT * FROM marketplace_settings').all() as Array<{ key: string; value: string }>;
    ...
    router.post('/api/marketplace/report-error', (req: Request, res: Response) => {
        ...
        const { appId, workspaceId, errorMessage, errorStack, context } = req.body as Record<string, unknown>;
    ...
    router.get('/api/marketplace/error-reports', (_req: Request, res: Response) => {
        const rows = db.prepare('SELECT * FROM marketplace_error_reports ORDER BY reported_at DESC LIMIT 200').all() as Array<Record<string, unknown>>;
        res.json(rows.map(r => ({ ...r, context: r.context ? JSON.parse(r.context as string) : null })));
    });

    router.get('/api/marketplace/telemetry-events', (_req: Request, res: Response) => {
        const rows = db.prepare('SELECT * FROM marketplace_telemetry ORDER BY recorded_at DESC LIMIT 500').all() as Array<Record<string, unknown>>;
        res.json(rows.map(r => ({ ...r, payload: r.payload ? JSON.parse(r.payload as string) : null })));
    });

    /* ── Stats ────────────────────────────────────────────────────────────── */
    router.get('/api/marketplace/stats', (_req: Request, res: Response) => {
        const totalTokens = (db.prepare('SELECT COUNT(*) as n FROM marketplace_tokens').get() as { n: number }).n;
        const activeTokens = (db.prepare("SELECT COUNT(*) as n FROM marketplace_tokens WHERE revoked = 0 AND redeemed_at IS NULL AND expires_at > datetime('now')").get() as { n: number }).n;
        const redeemedTokens = (db.prepare('SELECT COUNT(*) as n FROM marketplace_tokens WHERE redeemed_at IS NOT NULL').get() as { n: number }).n;
        const errorReports = (db.prepare('SELECT COUNT(*) as n FROM marketplace_error_reports').get() as { n: number }).n;
        const telemetryEvents = (db.prepare('SELECT COUNT(*) as n FROM marketplace_telemetry').get() as { n: number }).n;
