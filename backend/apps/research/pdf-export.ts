...
function hashSettings(
  ...
  const data = JSON.stringify({ settings, pf: topic.prepared_for, len: content.length });
...
function tokensToPdf(tokens: Tokens.Token[]): any[] {
  const content: any[] = [];

  for (const token of tokens) {
    switch (token.type) {
      case "heading": {
        const sizes: Record<number, number> = { 1: 20, 2: 16, 3: 14, 4: 13, 5: 12, 6: 11 };
        content.push({
        ...
        if (token.depth === 2) {
          content.push({
            canvas: [{ type: "line", x1: 0, y1: 0, x2: 460, y2: 0, lineWidth: 0.5, lineColor: "#e5e5e5" }],
            margin: [0, 0, 0, 8],
          });
        }
        break;
          ...
          for (const child of item.tokens || []) {
            if (child.type === "text" || child.type === "paragraph") {
              ...
              const nested = child.items.map((ni: any) => ({
                text: inlineTokensToPdf(ni.tokens?.[0]?.tokens || []),
                fontSize: 10,
                lineHeight: 1.3,
                color: "#333",
              }));
...
function inlineTokensToPdf(tokens: Tokens.Token[]): any[] {
  const parts: any[] = [];
    ...
    switch (t.type) {
      case "text":
        ...
        if ("tokens" in t && t.tokens) {
          parts.push(...inlineTokensToPdf(t.tokens as Tokens.Token[]));
        } else {
...
export async function generatePdf(input: PdfInput): Promise<Buffer> {
  ...
  if (hasBranding || preparedFor) {
    docContent.push({
      canvas: [{ type: "line", x1: 0, y1: 0, x2: 460, y2: 0, lineWidth: 1.5, lineColor: "#e5e5e5" }],
      margin: [0, 4, 0, 16],
    });
  }
  ...
  if (hasBranding) {
    ...
    const contactParts = [rs.company_name, rs.website, rs.contact_email].filter(Boolean);
  ...
  const docDefinition: any = {
    ...
    header: (currentPage: number) => {
      ...
      return {
        columns: [
          { text: headerLeft, fontSize: 7.5, color: "#9ca3af", bold: true, alignment: "left" },
          { text: headerRight, fontSize: 7.5, color: "#9ca3af", alignment: "right" },
        ],
        margin: [50, 20, 50, 0],
      };
    },
    ...
    footer: (currentPage: number, pageCount: number) => {
      ...
      if (hasBranding && rs.confidentiality_notice) {
        const notice = rs.confidentiality_notice.length > 80
