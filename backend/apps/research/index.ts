...
export function createRouter(db: InstanceType<typeof Database>) {
    ...
    router.put('/api/research/topics/:id', (req, res) => {
        ...
        `).run(
            ...
            revisit_interval !== undefined
                ? revisit_interval
                : existing.revisit_interval,
            status ?? existing.status,
            last_researched_at !== undefined
                ? last_researched_at
                : existing.last_researched_at,
            next_revisit_at !== undefined
                ? next_revisit_at
                : existing.next_revisit_at,
    ...
    router.get('/api/research/sessions/:id', (req, res) => {
        ...
        const findings = db
            .prepare(
                `SELECT * FROM research_findings WHERE session_id = ? ORDER BY created_at ASC`
            )
            .all(req.params.id);
        const report = db
    ...
    router.post('/api/research/sessions/:id/complete', (req, res) => {
        ...
        if (
            ...
            REVISIT_DAYS[topic.revisit_interval]
        ) {
    ...
    router.post('/api/research/topics/:id/synthesize', (req, res) => {
        ...
        if (existing)
            return res
                .status(409)
                .json({ error: 'Master synthesis already in progress' });
    ...
    router.post('/api/research/reports/:reportId/share', (req, res) => {
        ...
        db.prepare(
            `UPDATE research_reports SET share_token = ? WHERE id = ?`
        ).run(token, req.params.reportId);
    ...
    router.delete('/api/research/reports/:reportId/share', (req, res) => {
        db.prepare(
            `UPDATE research_reports SET share_token = NULL WHERE id = ?`
        ).run(req.params.reportId);
        res.json({ ok: true });
    });
