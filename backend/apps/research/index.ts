...
export function createRouter(db: InstanceType<typeof Database>) {
    ...
    router.post('/api/research/topics', (req, res) => {
        const { title, description, detail_level = 'standard', ongoing = 0, revisit_interval } = req.body;
    ...
    router.put('/api/research/topics/:id', (req, res) => {
        const { title, description, detail_level, ongoing, revisit_interval, status, last_researched_at, next_revisit_at } = req.body;
        const existing = db.prepare(`SELECT * FROM research_topics WHERE id = ?`).get(req.params.id) as any;
    ...
    router.get('/api/research/sessions/:id', (req, res) => {
        ...
        const findings = db.prepare(`SELECT * FROM research_findings WHERE session_id = ? ORDER BY created_at ASC`).all(req.params.id);
        const report = db.prepare(`SELECT * FROM research_reports WHERE session_id = ?`).get(req.params.id);
    ...
    router.post('/api/research/sessions/:id/findings', (req, res) => {
        ...
        res.status(201).json(db.prepare(`SELECT * FROM research_findings WHERE id = ?`).get(r.lastInsertRowid));
    });
    ...
    router.get('/api/research/queue', (_req, res) => {
        ...
        const dueForRevisit = db.prepare(`
            SELECT t.*
            FROM research_topics t
            WHERE t.ongoing = 1
    ...
    router.post('/api/research/sessions/:id/complete', (req, res) => {
        ...
        const session = db.prepare(`SELECT * FROM research_sessions WHERE id = ?`).get(req.params.id) as any;
        if (!session) return res.status(404).json({ error: 'Not found' });
        ...
        const topic = db.prepare(`SELECT * FROM research_topics WHERE id = ?`).get(session.topic_id) as any;
    ...
    router.post('/api/research/reports/:reportId/share', (req, res) => {
        const report = db.prepare(`SELECT * FROM research_reports WHERE id = ?`).get(req.params.reportId) as any;
        if (!report) return res.status(404).json({ error: 'Not found' });

        const token = report.share_token ?? crypto.randomUUID();
        db.prepare(`UPDATE research_reports SET share_token = ? WHERE id = ?`).run(token, req.params.reportId);
        res.json({ token, url: `/share/${token}` });
    });

    // Revoke share token
    router.delete('/api/research/reports/:reportId/share', (req, res) => {
        db.prepare(`UPDATE research_reports SET share_token = NULL WHERE id = ?`).run(req.params.reportId);
        res.json({ ok: true });
    });

    // Public: get report by share token (no auth required)
    router.get('/api/research/public/:token', (req, res) => {
        const report = db.prepare(`SELECT * FROM research_reports WHERE share_token = ?`).get(req.params.token) as any;
        if (!report) return res.status(404).json({ error: 'Not found' });

        const session = db.prepare(`SELECT * FROM research_sessions WHERE id = ?`).get(report.session_id) as any;
        const topic = db.prepare(`SELECT * FROM research_topics WHERE id = ?`).get(session.topic_id);
        const findings = db.prepare(`SELECT * FROM research_findings WHERE session_id = ? ORDER BY created_at ASC`).all(report.session_id);
