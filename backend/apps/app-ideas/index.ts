...
export function createRouter(db: InstanceType<typeof Database>) {
    ...
    router.get('/api/app-ideas', (_req, res) => {
        const ideas = db
            .prepare(`SELECT * FROM app_ideas ORDER BY group_name, name`)
            .all();
        const connections = db
            .prepare(`SELECT * FROM app_idea_connections`)
            .all();
    ...
    router.post('/api/app-ideas', (req, res) => {
        ...
        res.status(201).json(
            db
                .prepare(`SELECT * FROM app_ideas WHERE id=?`)
    ...
    router.put('/api/app-ideas/:id', (req, res) => {
        ...
        res.json(
            db.prepare(`SELECT * FROM app_ideas WHERE id=?`).get(req.params.id)
        );
    });

    router.patch('/api/app-ideas/:id/position', (req, res) => {
        const { pos_x, pos_y } = req.body;
        db.prepare(`UPDATE app_ideas SET pos_x=?,pos_y=? WHERE id=?`).run(
            pos_x,
            pos_y,
            req.params.id
        );
    ...
    router.post('/api/app-ideas/connections', (req, res) => {
        ...
        res.status(201).json(
            db
                .prepare(`SELECT * FROM app_idea_connections WHERE id=?`)
                .get(r.lastInsertRowid)
        );
    });

    router.delete('/api/app-ideas/connections/:id', (req, res) => {
        db.prepare(`DELETE FROM app_idea_connections WHERE id=?`).run(
            req.params.id
        );
        res.json({ ok: true });
    });
