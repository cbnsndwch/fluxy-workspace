...
export function createRouter(db: InstanceType<typeof Database>) {
    ...
    router.post('/api/app-ideas', (req, res) => {
        ...
        const r = db.prepare(`INSERT INTO app_ideas (name,description,stage,tags,group_name,color,pos_x,pos_y) VALUES (?,?,?,?,?,?,?,?)`)
            .run(name, description || null, stage || 'idea', JSON.stringify(tags || []), group_name || null, color || null, pos_x ?? 0, pos_y ?? 0);
        res.status(201).json(db.prepare(`SELECT * FROM app_ideas WHERE id=?`).get(r.lastInsertRowid));
    });

    router.put('/api/app-ideas/:id', (req, res) => {
        const { name, description, stage, tags, group_name, color, pos_x, pos_y } = req.body;
        db.prepare(`UPDATE app_ideas SET name=?,description=?,stage=?,tags=?,group_name=?,color=?,pos_x=?,pos_y=?,updated_at=datetime('now') WHERE id=?`)
            .run(name, description || null, stage || 'idea', JSON.stringify(tags || []), group_name || null, color || null, pos_x ?? 0, pos_y ?? 0, req.params.id);
    ...
    router.post('/api/app-ideas/connections', (req, res) => {
        ...
        const r = db.prepare(`INSERT INTO app_idea_connections (source_id,target_id,label,strength) VALUES (?,?,?,?)`)
            .run(source_id, target_id, label || null, strength || 1);
        res.status(201).json(db.prepare(`SELECT * FROM app_idea_connections WHERE id=?`).get(r.lastInsertRowid));
    });
