...
function gitOr(workspace: string, args: string, fallback = ''): string {
    try { return git(workspace, args); } catch { return fallback; }
}
...
function parseStatus(workspace: string) {
    ...
    for (const line of lines) {
        if (line.startsWith('# branch.head ')) {
            ...
            if (xy[0] !== '.') staged.push({ code: xy[0], path: newPath, oldPath });
...
function parseLog(workspace: string, limit: number, skip: number) {
    ...
    const raw = gitOr(workspace, `log --format="${fmt}%x1e" --max-count=${limit} --skip=${skip}`);
    if (!raw) return [];
    ...
    return raw
        ...
        .filter(Boolean)
        .map(block => {
            const [sha, shortSha, subject, authorName, authorEmail, date, parentsRaw] = block.split(SEP);
            return {
                sha:         sha.trim(),
...
function parseBranches(workspace: string) {
    ...
    return raw.split('\n').filter(Boolean).map(line => {
        const [head, name, sha, subject, upstreamShort, upstreamTrack] = line.split(SEP);
        return {
            name:          name.trim(),
            isCurrent:     head.trim() === '*',
...
function parseWorktrees(workspace: string) {
    ...
    for (const line of raw.split('\n')) {
        if (line.startsWith('worktree ')) {
            ...
            cur = { path: line.slice(9), sha: '', branch: null, isMain: first, isBare: false, isLocked: false };
            first = false;
...
function getCommitDetail(workspace: string, sha: string) {
    ...
    const meta = gitOr(workspace, `show --no-patch --format="${fmt}%x1e" ${sha}`)
        .split('\x1e')[0]?.trim() ?? '';

    const [fullSha, shortSha, subject, body, authorName, authorEmail, date, parentsRaw] = meta.split(SEP);

    // File stats
    const stats = gitOr(workspace, `show --stat --no-patch ${sha}`);

    // Unified diff — cap at 300 KB
    let diff = gitOr(workspace, `show --unified=3 --no-color --format="" ${sha}`);
    const MAX = 300_000;
...
export function createRouter(workspace: string) {
    ...
    router.get('/api/git/log', (req, res) => {
        ...
        try {
            ...
            res.json({ commits: commits.slice(0, limit), skip, limit, hasMore });
        } catch (err) {
            res.status(500).json({ error: String(err) });
        }
    });
    ...
    router.get('/api/git/uncommitted', (_req, res) => {
        try {
            const staged   = gitOr(workspace, 'diff --cached --unified=3 --no-color');
