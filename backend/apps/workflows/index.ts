...
function makeRunner(db: InstanceType<typeof Database>) {
  return async function runWorkflow(
    workflowId: string,
    emit: (
      e: NodeEvent | { type: "run_complete" | "run_error"; run_id: number; error?: string },
    ) => void,
  ): Promise<number> {
    ...
    const r = db
      .prepare(`INSERT INTO workflow_runs (workflow_id, status) VALUES (?, 'running')`)
      .run(workflowId);
    ...
    const finalStatus = sorted.some((n) => {
      const row = db
        .prepare(`SELECT status FROM workflow_run_nodes WHERE run_id=? AND node_id=?`)
    ...
    db.prepare(`UPDATE workflow_runs SET status=?, finished_at=datetime('now') WHERE id=?`).run(
      finalStatus,
      runId,
    );
...
export function createRouter(db: InstanceType<typeof Database>) {
  ...
  router.get("/api/workflow-runs/:runId", (req, res) => {
    ...
    const nodes = db
      .prepare(`SELECT * FROM workflow_run_nodes WHERE run_id=? ORDER BY id`)
