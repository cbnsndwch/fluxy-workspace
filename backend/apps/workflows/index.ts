...
function runInSandbox(
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            safeInput = JSON.parse(JSON.stringify(input ?? null));
        } catch {
            safeInput = null;
        }
        try {
            safeCtx = JSON.parse(JSON.stringify(context));
        } catch {
            safeCtx = {};
        }
        ...
        worker.on(
            'message',
            (msg: { type: string; value?: unknown; message?: string }) => {
                if (msg.type === 'result') {
                    clearTimeout(timer);
                    worker.terminate();
                    resolve(msg.value);
        ...
        worker.on('exit', code => {
            clearTimeout(timer);
            if (code !== 0) reject(new Error(`Sandbox exited (${code})`));
        });
    });
}
...
async function executeNode(
    ...
    switch (node.type) {
        ...
        case 'cron_trigger': {
            try {
                const parsed = cfg.initial_data
                    ? JSON.parse(cfg.initial_data)
                    : {};
                ctx.$input = parsed;
                ctx.trigger = parsed;
        ...
        case 'db_query': {
            ...
            if (cfg.params) {
                try {
                    params = JSON.parse(interpolate(cfg.params, ctx));
                } catch {
        ...
        case 'condition': {
            let safeInput: unknown, safeCtx: unknown;
            try {
                safeInput = JSON.parse(JSON.stringify(ctx.$input ?? null));
            } catch {
                safeInput = null;
            }
            try {
...
function makeRunner(db: InstanceType<typeof Database>) {
    return async function runWorkflow(
        ...
        const wf = db
            ...
            .get(workflowId) as { nodes: string; edges: string } | undefined;
        ...
        const insertNode = db.prepare(
            `INSERT INTO workflow_run_nodes (run_id, node_id, node_type, status) VALUES (?, ?, ?, 'pending')`
        );
        ...
        for (const node of sorted) {
            if (skipped.has(node.id)) {
                db.prepare(
                    `UPDATE workflow_run_nodes SET status='skipped' WHERE run_id=? AND node_id=?`
                ).run(runId, node.id);
            ...
            db.prepare(
                `UPDATE workflow_run_nodes SET status='running', executed_at=datetime('now') WHERE run_id=? AND node_id=?`
            ).run(runId, node.id);
            ...
            db.prepare(
                `UPDATE workflow_run_nodes SET status=?, output=?, error=?, duration_ms=? WHERE run_id=? AND node_id=?`
            ).run(
                nodeStatus,
        ...
        const finalStatus = sorted.some(n => {
            const row = db
                ...
                .get(runId, n.id) as { status: string } | undefined;
        ...
        emit({
            type: finalStatus === 'error' ? 'run_error' : 'run_complete',
            run_id: runId
        });
        return runId;
    };
...
function startCronScheduler(
    ...
    async function checkCrons() {
        ...
        try {
            workflows = db.prepare(`SELECT id, nodes FROM workflows`).all() as {
                id: string;
                nodes: string;
            }[];
        } catch {
        ...
        for (const wf of workflows) {
            ...
            const schedule = (
                (cronNode.data as Record<string, string>).schedule || ''
            ).trim();
            ...
            try {
                ...
                if (now.getTime() - prevOccurrence.getTime() < 60_000) {
                    ...
                    runner(wf.id, () => {}).catch((e: Error) =>
                        console.error(
                            `[cron] Workflow ${wf.id} failed:`,
                            e.message
                        )
                    );
                    console.log(
                        `[cron] Triggered workflow ${wf.id} (schedule: ${schedule})`
                    );
                }
            } catch {
...
export function createRouter(db: InstanceType<typeof Database>) {
    ...
    router.post('/api/workflows', (req, res) => {
        ...
        ).run(id, name || 'Untitled Workflow', description || null);
        res.status(201).json(
    ...
    router.put('/api/workflows/:id', (req, res) => {
        ...
        db.prepare(
            `UPDATE workflows SET name=?, description=?, nodes=?, edges=?, updated_at=datetime('now') WHERE id=?`
        ).run(
    ...
    router.get('/api/workflow-runs/:runId', (req, res) => {
        ...
        const nodes = db
            .prepare(
                `SELECT * FROM workflow_run_nodes WHERE run_id=? ORDER BY id`
            )
