...
export function createRouter(db: Database.Database) {
    ...
    router.get('/api/schedules/processes', (_req, res) => {
        const processes: {
            ...
            vsz: string; rss: string; stat: string; started: string; time: string; command: string;
        }[] = [];
    ...
    router.get('/api/schedules/runs', (req, res) => {
        ...
        if (cronId) {
            res.json(db.prepare(
                'SELECT * FROM cron_runs WHERE cron_id = ? ORDER BY started_at DESC LIMIT ?'
    ...
    router.post('/api/schedules/runs', (req, res) => {
        ...
        const result = db.prepare(
            'INSERT INTO cron_runs (cron_id, trigger, stdout, stderr) VALUES (?, ?, ?, ?)'
    ...
    router.patch('/api/schedules/runs/:id', (req, res) => {
        ...
        if (finished_at !== undefined) { sets.push('finished_at = ?'); vals.push(finished_at); }
        // Append mode for stdout/stderr — useful for streaming output
        if (stdout !== undefined) { sets.push('stdout = stdout || ?'); vals.push(stdout); }
        if (stderr !== undefined) { sets.push('stderr = stderr || ?'); vals.push(stderr); }
        if (exit_code !== undefined) { sets.push('exit_code = ?'); vals.push(exit_code); }
