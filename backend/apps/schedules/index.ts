...
export function createRouter(db: Database.Database) {
    ...
    router.get('/api/schedules/crons', (_req, res) => {
        ...
        res.json(
            crons.map(c => ({
                ...c,
                nextRun: c.enabled ? getNextRun(c.schedule) : null
            }))
        );
    });
    ...
    router.post('/api/schedules/crons', (req, res) => {
        ...
        if (!id || !schedule || !task) {
            return res
                .status(400)
                .json({ error: 'id, schedule, and task are required' });
        }
        const crons = readCrons();
        if (crons.find(c => c.id === id)) {
            return res
                .status(409)
                .json({ error: `A cron with id '${id}' already exists` });
        }
    ...
    router.post('/api/schedules/runs', (req, res) => {
        ...
        if (!cron_id)
            return res.status(400).json({ error: 'cron_id is required' });
        const result = db
            .prepare(
    ...
    router.patch('/api/schedules/runs/:id', (req, res) => {
        ...
        if (sets.length > 0) {
            vals.push(id);
            db.prepare(
                `UPDATE cron_runs SET ${sets.join(', ')} WHERE id = ?`
            ).run(...vals);
        }
