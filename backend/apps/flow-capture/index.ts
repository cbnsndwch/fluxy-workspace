...
export function createRouter(db: Database) {
    ...
    router.post('/api/flow-capture/sessions', (req, res) => {
        ...
        const result = db
            .prepare(`INSERT INTO flow_sessions (name) VALUES (?)`)
            .run(name);
        res.json({
    ...
    router.get('/api/flow-capture/sessions/:id', (req, res) => {
        const session = db
            .prepare(`SELECT * FROM flow_sessions WHERE id = ?`)
            .get(req.params.id);
        if (!session) return res.status(404).json({ error: 'Not found' });

        const chunks = db
            .prepare(
                `SELECT * FROM flow_chunks WHERE session_id = ? ORDER BY sequence ASC`
            )
            .all(req.params.id);
        ...
        const latest = db
            ...
            | { mermaid: string; chunk_node_map?: string }
            | undefined;
        ...
        res.json({
            ...
            chunkNodeMap: latest?.chunk_node_map
                ? JSON.parse(latest.chunk_node_map)
                : {}
        });
    });
    ...
    router.patch('/api/flow-capture/sessions/:id', (req, res) => {
        ...
        if (name)
            db.prepare(`UPDATE flow_sessions SET name = ? WHERE id = ?`).run(
                name,
                req.params.id
            );
        res.json({ ok: true });
    ...
    router.post('/api/flow-capture/sessions/:id/chunks', (req, res) => {
        ...
        const { n } = db
            .prepare(
                `SELECT COUNT(*) AS n FROM flow_chunks WHERE session_id = ?`
            )
            .get(req.params.id) as { n: number };

        const result = db
            .prepare(
                `INSERT INTO flow_chunks (session_id, text, sequence) VALUES (?, ?, ?)`
            )
            .run(req.params.id, text, n + 1);

        db.prepare(
            `UPDATE flow_sessions SET updated_at = datetime('now') WHERE id = ?`
        ).run(req.params.id);
    ...
    router.patch(
        '/api/flow-capture/sessions/:id/chunks/:chunkId',
        (req, res) => {
            const { text } = req.body as { text?: string };
            if (!text?.trim())
                return res.status(400).json({ error: 'text required' });
            db.prepare(
                `UPDATE flow_chunks SET text = ? WHERE id = ? AND session_id = ?`
            ).run(text.trim(), req.params.chunkId, req.params.id);
            res.json({ ok: true });
        }
    );
    ...
    router.delete(
        ...
        (req, res) => {
            db.prepare(
                `DELETE FROM flow_chunks WHERE id = ? AND session_id = ?`
            ).run(req.params.chunkId, req.params.id);
            res.json({ ok: true });
        }
    );
    ...
    router.post('/api/flow-capture/sessions/:id/analyze', async (req, res) => {
        ...
        try {
            ...
            res.json({
                complete: Array.isArray(parsed.complete)
                    ? parsed.complete.filter(Boolean)
                    : [],
                remainder:
                    typeof parsed.remainder === 'string' ? parsed.remainder : ''
            });
        } catch (err) {
    ...
    router.put('/api/flow-capture/sessions/:id/diagram', (req, res) => {
        ...
        if (!code?.trim())
            return res.status(400).json({ error: 'mermaid required' });
        db.prepare(
            `INSERT INTO flow_diagrams (session_id, mermaid) VALUES (?, ?)`
        ).run(req.params.id, code.trim());
        db.prepare(
            `UPDATE flow_sessions SET updated_at = datetime('now') WHERE id = ?`
        ).run(req.params.id);
        res.json({ ok: true });
    ...
    router.post('/api/flow-capture/sessions/:id/diagram', async (req, res) => {
        const chunks = db
            .prepare(
                `SELECT text FROM flow_chunks WHERE session_id = ? ORDER BY sequence ASC`
            )
            .all(req.params.id) as { text: string }[];

        if (!chunks.length)
            return res.status(400).json({ error: 'No chunks in session' });
        ...
        try {
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    ...
                    {
                        role: 'user',
                        content: buildPrompt(chunks, currentDiagram, remix)
                    }
                ],
                max_tokens: 1500,
                temperature: 0.2,
            ...
            const mermaid = sanitize(
                typeof raw.diagram === 'string' ? raw.diagram : ''
            );
            const chunkNodeMap: Record<string, string[]> =
                raw.chunkNodeMap && typeof raw.chunkNodeMap === 'object'
                    ? raw.chunkNodeMap
                    : {};
    ...
    router.post('/api/flow-capture/sessions/:id/title', async (req, res) => {
        const chunks = db
            .prepare(
                `SELECT text FROM flow_chunks WHERE session_id = ? ORDER BY sequence ASC LIMIT 10`
            )
            .all(req.params.id) as { text: string }[];

        if (!chunks.length)
            return res.status(400).json({ error: 'No chunks to summarize' });
