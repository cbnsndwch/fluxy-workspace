...
export function createRouter(db: Database) {
    ...
    try { db.exec(`ALTER TABLE flow_diagrams ADD COLUMN chunk_node_map TEXT`); } catch { /* already exists */ }
    ...
    router.post('/api/flow-capture/sessions/:id/chunks', (req, res) => {
        ...
        db.prepare(`UPDATE flow_sessions SET updated_at = datetime('now') WHERE id = ?`).run(req.params.id);
    ...
    router.patch('/api/flow-capture/sessions/:id/chunks/:chunkId', (req, res) => {
        ...
        db.prepare(`UPDATE flow_chunks SET text = ? WHERE id = ? AND session_id = ?`).run(text.trim(), req.params.chunkId, req.params.id);
        res.json({ ok: true });
    });

    router.delete('/api/flow-capture/sessions/:id/chunks/:chunkId', (req, res) => {
        db.prepare(`DELETE FROM flow_chunks WHERE id = ? AND session_id = ?`).run(req.params.chunkId, req.params.id);
        res.json({ ok: true });
    });
    ...
    router.put('/api/flow-capture/sessions/:id/diagram', (req, res) => {
        ...
        db.prepare(`INSERT INTO flow_diagrams (session_id, mermaid) VALUES (?, ?)`).run(req.params.id, code.trim());
        db.prepare(`UPDATE flow_sessions SET updated_at = datetime('now') WHERE id = ?`).run(req.params.id);
        res.json({ ok: true });
    });
    ...
    router.post('/api/flow-capture/sessions/:id/diagram', async (req, res) => {
        ...
        const { currentDiagram, remix = false } = req.body as { currentDiagram?: string; remix?: boolean };
        ...
        try {
            ...
            db.prepare(`INSERT INTO flow_diagrams (session_id, mermaid, chunk_node_map) VALUES (?, ?, ?)`)
    ...
    router.post('/api/flow-capture/sessions/:id/title', async (req, res) => {
        ...
        try {
            const completion = await openai.chat.completions.create({
                model: 'gpt-4o-mini',
                messages: [
                    { role: 'system', content: 'You generate short, descriptive session titles. Return ONLY the title — 3 to 5 words, no punctuation, no quotes, no explanation. Capture the core topic or flow being described.' },
            ...
            db.prepare(`UPDATE flow_sessions SET name = ?, updated_at = datetime('now') WHERE id = ?`).run(name, req.params.id);
            res.json({ name });
        } catch (err) {
    ...
    router.post('/api/flow-capture/diagram', async (req, res) => {
        const { transcript, currentDiagram } = req.body as { transcript?: string; currentDiagram?: string };
