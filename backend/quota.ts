...
function parseUnifiedHeaders(
    allHeaders: Record<string, string>,
    model: string
): UnifiedQuotaSnapshot {
    const h = (key: string) =>
        allHeaders[`anthropic-ratelimit-unified-${key}`] ?? null;
    const hNum = (key: string) => {
        const v = h(key);
    ...
    return {
        model,
        unified_status:
            allHeaders['anthropic-ratelimit-unified-status'] ?? null,
        representative_claim:
            allHeaders['anthropic-ratelimit-unified-representative-claim'] ??
            null,
        ...
        upgrade_paths:
            allHeaders['anthropic-ratelimit-unified-upgrade-paths'] ?? null,
        source: 'passive',
...
function computeRecommendation(snap: UnifiedQuotaSnapshot | null): {
    ...
    if (!snap) {
        return {
            ...
            reason: 'No quota data — assume fresh capacity',
            details: {}
        };
    }

    const capturedISO = snap.captured_at.endsWith('Z')
        ? snap.captured_at
        : snap.captured_at + 'Z';
    ...
    if (snap.unified_status === 'throttled') {
        const resetAt = snap.five_h_reset
            ? new Date(snap.five_h_reset * 1000)
            : null;
        const minsUntilReset = resetAt
            ? Math.max(0, Math.round((resetAt.getTime() - Date.now()) / 60_000))
            : null;
    ...
    if (fiveH != null) {
        ...
        if (fiveH < 0.4) {
            return {
                recommendation: 'aggressive',
                reason: `5h window at ${pctUsed}% — plenty of headroom`,
                details
            };
        }
        if (fiveH < 0.7) {
            return {
                ...
                reason: `5h window at ${pctUsed}% — pace yourself`,
                details
            };
        }
        if (fiveH < 0.9) {
            return {
                recommendation: 'cautious',
                reason: `5h window at ${pctUsed}% — approaching warning threshold`,
                details
            };
        }
        ...
        return {
            recommendation: 'cautious',
            reason: `5h window at ${pctUsed}% — very close to limit`,
            details
        };
    }
    ...
    if (stale) {
        return {
            recommendation: 'aggressive',
            reason: `No recent API activity (${ageMins}min ago) — capacity likely refreshed`,
            details
        };
    }
    ...
    return {
        recommendation: 'moderate',
        reason: 'Quota data present but utilization unknown',
        details
    };
}
...
function getActivityContext(): {
    ...
    if (hour >= 23 || hour < 7)
        return {
            period: 'sleeping',
            hint: 'Human likely asleep — prime time for batch work'
        };
    if (hour >= 9 && hour < 18)
        return {
            period: 'active',
            hint: 'Human likely active — share quota conservatively'
        };
    return {
        period: 'quiet',
        hint: 'Outside core hours — moderate batch work OK'
    };
}
...
function writeSnapshot(db: Database.Database, snap: UnifiedQuotaSnapshot) {
  ...
  `).run(
        snap.model,
        snap.unified_status,
        snap.representative_claim,
        snap.five_h_utilization,
        snap.five_h_status,
        snap.five_h_reset,
        snap.five_h_surpassed_threshold,
        snap.seven_d_utilization,
        snap.seven_d_status,
        snap.seven_d_reset,
        snap.fallback_available,
        snap.fallback_percentage,
        snap.overage_in_use ? 1 : 0,
        snap.overage_status,
        snap.overage_utilization,
        snap.overage_reset,
        snap.upgrade_paths,
        snap.source
    );
    ...
    db.prepare(
        `DELETE FROM api_quota WHERE id NOT IN (SELECT id FROM api_quota ORDER BY id DESC LIMIT 1000)`
    ).run();
}

function getLatest(db: Database.Database): UnifiedQuotaSnapshot | null {
    return db
        .prepare(`SELECT * FROM api_quota ORDER BY id DESC LIMIT 1`)
        .get() as UnifiedQuotaSnapshot | null;
}
...
function getHistory(
    ...
    return db
        .prepare(`SELECT * FROM api_quota ORDER BY id DESC LIMIT ?`)
        .all(limit) as UnifiedQuotaSnapshot[];
}
...
async function runProbe(db: Database.Database) {
    try {
        ...
        const hasUnified = Object.keys(headers).some(k =>
        ...
        if (hasUnified) {
            ...
            const u5h =
                snap.five_h_utilization != null
                    ? `${Math.round(snap.five_h_utilization * 100)}%`
                    : '?';
            const u7d =
                snap.seven_d_utilization != null
                    ? `${Math.round(snap.seven_d_utilization * 100)}%`
                    : '?';
            console.log(
                `[quota] Probe: 5h=${u5h} 7d=${u7d} status=${snap.unified_status} fallback=${snap.fallback_available}`
            );
        } else {
            console.log(
                `[quota] Probe: no unified headers (status ${resp.status})`
            );
        }
    } catch (err: any) {
...
export function initQuota(db: Database.Database): Router {
    ...
    r.get('/api/quota', (_req: Request, res: Response) => {
        ...
        const { recommendation, reason, details } =
            computeRecommendation(latest);
        const activity = getActivityContext();

        // captured_at is stored via SQLite datetime('now') which is UTC — append Z for correct parsing
        const capturedISO = latest?.captured_at?.endsWith('Z')
            ? latest.captured_at
            : latest?.captured_at + 'Z';
        const ageMs = latest
            ? Date.now() - new Date(capturedISO).getTime()
            : null;
        ...
        res.json({
            ...
            details: {
                ...details,
                five_h_reset_at: latest?.five_h_reset
                    ? new Date(latest.five_h_reset * 1000).toISOString()
                    : null,
                seven_d_reset_at: latest?.seven_d_reset
                    ? new Date(latest.seven_d_reset * 1000).toISOString()
                    : null
            },
            current: latest
                ? {
                      ...
                      five_h: {
                          ...
                          reset_at: latest.five_h_reset
                              ? new Date(
                                    latest.five_h_reset * 1000
                                ).toISOString()
                              : null,
                      ...
                      seven_d: {
                          ...
                          reset_at: latest.seven_d_reset
                              ? new Date(
                                    latest.seven_d_reset * 1000
                                ).toISOString()
                              : null
                      },
                      fallback: {
