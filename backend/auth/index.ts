...
export function getSessionUser(db: InstanceType<typeof Database>, req: Request): DbUser | null {
...
export function createRouter(db: InstanceType<typeof Database>) {
    ...
    const GITHUB_REDIRECT_URI = process.env.GITHUB_REDIRECT_URI || 'http://localhost:3000/app/api/auth/github/callback';

    router.get('/api/auth/github', (_req, res) => {
        if (!GITHUB_CLIENT_ID) return res.status(500).json({ error: 'GitHub OAuth not configured. Set GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET in .env' });
    ...
    router.get('/api/auth/github/callback', async (req, res) => {
        ...
        try {
            ...
            `).run(ghUser.id, ghUser.login, ghUser.name ?? null, ghUser.avatar_url ?? null, ghUser.email ?? null);

            const user = db.prepare('SELECT * FROM users WHERE github_id = ?').get(ghUser.id) as DbUser;
            ...
            res.setHeader('Set-Cookie', `session=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${7 * 24 * 3600}`);
