...
export default function MarbleStudioPage() {
    const { trackPageView, trackAction: track } =
        useAppTracking('marble-studio');
    ...
    const selectedWorldId = worldMatch
        ? parseInt(worldMatch.params.worldId!)
        : null;

    const loadApiKeyStatus = useCallback(async () => {
        const res = await fetch('/app/api/marble-studio/settings').catch(
            () => null
        );
        if (res?.ok) {
    ...
    const handleSync = useCallback(async () => {
        ...
        try {
            ...
            setSyncMsg({
                text:
                    synced > 0
                        ? `Synced ${synced} world${synced !== 1 ? 's' : ''}`
                        : 'Already up to date',
                ok: true
            });
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button
                                    ...
                                    onClick={() =>
                                        navigate('/marble-studio/settings')
                                    }
                                >
