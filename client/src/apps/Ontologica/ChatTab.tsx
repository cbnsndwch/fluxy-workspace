...
export function ChatTab() {
    const { projectId, loadGraph, loadStats, loadProject } =
        useProjectContext();
    const onGraphChange = () => {
        loadGraph();
        loadStats();
    ...
    const [, setLastActions] = useState<AppliedAction[]>([]);
    ...
    const loadConversations = useCallback(async () => {
        const res = await fetch(
            `/app/api/ontologica/projects/${projectId}/conversations`
        );
        if (res.ok) {
    ...
    const handleSend = async () => {
        ...
        try {
            const res = await fetch(
                `/app/api/ontologica/projects/${projectId}/chat`,
                {
                    method: 'POST',
    ...
    const handleClearChat = async () => {
        await fetch(`/app/api/ontologica/projects/${projectId}/conversations`, {
            method: 'DELETE'
        });
        setMessages([]);
        setLastActions([]);
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.length === 0 && (
                    <div className="flex-1 flex items-center justify-center h-full min-h-75">
                        <div className="text-center max-w-md">
                            ...
                            <p className="text-sm text-muted-foreground mb-6">
                                Tell me how your business works in plain
                                language. I'll extract the concepts,
                                relationships, and structure — building your
                                knowledge map as we talk.
                            </p>
            ...
            <div className="p-4 border-t">
                <div className="flex items-end gap-2">
                    ...
                    <Textarea
                        ...
                        rows={1}
                        className="resize-none min-h-10.5 max-h-30"
                        disabled={sending}
                    />
                    <Button
                        size="icon"
