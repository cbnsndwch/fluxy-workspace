...
export function ForceGraphTab() {
    ...
    const [filterEdgeTypes, setFilterEdgeTypes] = useState<Set<string>>(
        new Set(['is_a', 'object_property', 'data_property'])
    );
    ...
    const catColors = useMemo(
        () => buildCategoryColorMap(rawNodes, nodeMap),
        [rawNodes, nodeMap]
    );
            ...
            for (const nid of expandedNodes) {
                ...
                for (const e of rawEdges) {
                    if (e.source_node_id === nid)
                        visibleIds.add(e.target_node_id);
                    if (e.target_node_id === nid)
                        visibleIds.add(e.source_node_id);
                }
            }
        }
    ...
    const paintNode = useCallback(
        (node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
            ...
            const color = isBaseLayer
                ? '#06b6d4'
                : catColors.get(node.__category || 'Uncategorized') ||
                  '#6b7280';
            ...
            if (globalScale > 1.5 || isSelected || hoverNode?.id === node.id) {
                ...
                if (
                    isBaseLayer &&
                    node.__layerName &&
                    (isSelected || hoverNode?.id === node.id)
                ) {
    ...
    const paintLink = useCallback(
        (link: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
            ...
            const key = `${sid}-${tid}`;
            const isHighlighted =
                highlightedLinks.size === 0 || highlightedLinks.has(key);
            const alpha = isHighlighted ? 0.6 : 0.08;
            const connectsBaseLayer =
                baseLayerNodeIds.has(sid) || baseLayerNodeIds.has(tid);
            ...
            ctx.strokeStyle = connectsBaseLayer
                ? `#06b6d4${Math.round(alpha * 255)
                      .toString(16)
                      .padStart(2, '0')}`
                : `${EDGE_COLORS[link.edge_type] || '#6b7280'}${Math.round(
                      alpha * 255
                  )
                      .toString(16)
                      .padStart(2, '0')}`;
        ...
        return (
            <div className="flex-1 flex items-center justify-center text-muted-foreground">
                <div className="text-center space-y-2">
                    ...
                    <p className="text-sm">
                        Upload documents and run extraction to build the graph
                    </p>
                </div>
            </div>
        );
    ...
    return (
        <div className="w-full h-full flex overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                {showFilters && (
                    <div className="shrink-0 flex items-center gap-3 px-3 py-2 border-b border-border/30 bg-muted/30">
                        <div className="flex items-center gap-1.5 text-[11px]">
                            ...
                            {(['all', 'class', 'individual'] as const).map(
                                t => (
                                    <button
                                        key={t}
                        ...
                        <div className="flex items-center gap-1.5 text-[11px]">
                            ...
                            {['is_a', 'object_property', 'data_property'].map(
                                t => (
                                    <button
                                        key={t}
                ...
                <div
                    ref={containerRef}
                    className="flex-1 min-h-0 bg-background relative"
                >
                    {/* Legend overlay */}
                    {showLegend && (
                        <div className="absolute bottom-3 left-3 z-10 bg-background/90 backdrop-blur-sm border border-border/40 rounded-md px-3 py-2 space-y-1.5">
                            ...
                            {baseLayerNodeIds.size > 0 && (
                                <div className="flex items-center gap-2 text-[10px] text-cyan-400/80">
                                    ...
                                    <span>
                                        Dashed edge — connects to base layer
                                    </span>
                                </div>
                            )}
                            {/* Active layers list */}
                            {layerMap.size > 0 && (
                                <>
                                    ...
                                    {Array.from(new Set(layerMap.values())).map(
                                        name => (
                                            <div
                                                key={name}
                                                className="flex items-center gap-2 text-[10px] text-cyan-400/70"
                                            >
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-3">
                        <div>
                            ...
                            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                                <Badge
                                    variant="outline"
                                    className={`text-[9px] ${selectedNode.node_type === 'class' ? 'text-emerald-400 border-emerald-500/30' : 'text-violet-400 border-violet-500/30'}`}
                                >
                                    {selectedNode.node_type}
                                </Badge>
                                <Badge
                                    variant="outline"
                                    className={`text-[9px] ${selectedNode.status === 'approved' ? 'text-emerald-400 border-emerald-500/30' : 'text-amber-400 border-amber-500/30'}`}
                                >
                                    {selectedNode.status}
                                </Badge>
                                {selectedNode.__isBaseLayer &&
                                    selectedNode.__layerName && (
                                        <Badge
                                            variant="outline"
                                            className="text-[9px] text-cyan-400 border-cyan-500/30 border-dashed"
                                        >
                                            {selectedNode.__layerName}
                                        </Badge>
                                    )}
                        ...
                        {selectedNode.base_item_uri && (
                            <>
                                ...
                                <div>
                                    ...
                                    <p className="text-[9px] text-cyan-400/80 font-mono break-all">
                                        {selectedNode.base_item_uri}
                                    </p>
                                </div>
                            </>
                                        ...
                                        {detailEdges.map((e: any) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <span
                                                        className={`font-medium shrink-0 ${isObjProp ? 'text-violet-400' : 'text-amber-400'}`}
                                                    >
                                                        {e.name || e.edge_type}
                                                    </span>
