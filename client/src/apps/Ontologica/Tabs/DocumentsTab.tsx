...
export function DocumentsTab() {
    ...
    const handleDelete = async (docId: number) => {
        await fetch(`/app/api/ontologica/documents/${docId}`, {
            method: 'DELETE'
        });
        onDocumentsChange();
    };
    ...
    const saveOrder = useCallback(
        async (ids: number[]) => {
            await fetch(
                `/app/api/ontologica/projects/${projectId}/documents/reorder`,
                {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
    ...
    const handleReorderDrop = useCallback(
        ...
        [documents, onDocumentsChange, saveOrder]
    );
        ...
        <div className="p-6 space-y-4 h-full overflow-y-auto">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-medium text-muted-foreground">
                    Source Documents — drag to reorder, click extract to build
                    knowledge
                </h3>
            ...
            {documents.length === 0 && (
                ...
                >
                    ...
                    <p className="text-xs mt-1">
                        Upload files, drag & drop, or paste text
                    </p>
                </button>
            )}
                    ...
                    >
                        <CardContent className="p-4 flex flex-col gap-3">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <GripVertical
                                    size={14}
                                    className="text-muted-foreground/30 cursor-grab active:cursor-grabbing shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
                                />
                            </div>
                            ...
                            <div className="flex items-center justify-between border-t border-border/40 pt-2.5">
                                ...
                                <div className="flex items-center gap-0.5">
                                    <Button
                                        ...
                                        onClick={() =>
                                            navigate(
                                                `/ontologica/${projectId}/documents/preview/${doc.id}`
                                            )
                                        }
            ...
            >
                <DialogContent className="sm:max-w-lg overflow-hidden">
                    ...
                    >
                        <Upload
                            size={32}
                            className={`mx-auto mb-2 ${dragOver ? 'text-emerald-500 animate-bounce' : 'text-muted-foreground/40'}`}
                        />
                        ...
                        <div className="flex items-center gap-3 mt-4 justify-center">
                            ...
                            >
                                <FileText size={14} className="mr-1" /> Paste
                                Text
                            </Button>
                        </div>
                    </div>
