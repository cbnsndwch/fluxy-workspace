    ...
    function build(id: number, depth: number): TreeNode {
        ...
        kids.sort((a, b) => {
            ...
            if (a.node_type !== b.node_type)
                return a.node_type === 'class' ? -1 : 1;
            return a.name.localeCompare(b.name);
        });
        const descendantCount = kids.reduce(
            (s, k) => s + 1 + k.descendantCount,
            0
        );
...
function deduplicateRoots(roots: TreeNode[]): {
    categories: TreeNode[];
    orphans: TreeNode[];
} {
        ...
        if (root.children.length > 0) {
            ...
            if (seen.has(key)) {
                ...
                if (root.descendantCount > existing.descendantCount) {
                    ...
                    root.descendantCount += existingChildren.reduce(
                        (s, c) => s + 1 + c.descendantCount,
                        0
                    );
                    ...
                    existing.descendantCount += newChildren.reduce(
                        (s, c) => s + 1 + c.descendantCount,
                        0
                    );
                }
...
function CategoryCard({
    ...
    const childClasses = node.children.filter(
        c => c.node_type === 'class'
    ).length;
    const childInstances = node.children.filter(
        c => c.node_type === 'individual'
    ).length;
        ...
        >
            ...
            <div className="flex items-start justify-between gap-2 mb-3">
                <div className="flex items-center gap-2 min-w-0">
                    <div
                        className={`shrink-0 w-8 h-8 rounded-md ${color.bg} flex items-center justify-center`}
                    >
                ...
                <ChevronRight
                    size={16}
                    className="shrink-0 text-muted-foreground/50 group-hover:text-foreground/70 transition-colors mt-1"
                />
            </div>
            ...
            <div className="mt-3 h-[3px] bg-white/5 rounded-full overflow-hidden">
                <div
                    ...
                    style={{ width: `${node.confidence * 100}%`, opacity: 0.6 }}
                />
            </div>
        </button>
    );
    ...
    return (
        <>
          ...
          ${isSelected ? 'bg-emerald-500/15 text-foreground' : 'hover:bg-white/5 text-foreground/80'}`}
                ...
                title={
                    isBaseLayer
                        ? `Base layer: ${layerName}${node.base_item_uri ? `\nURI: ${node.base_item_uri}` : ''}`
                        : undefined
                }
            >
                {/* Expand toggle */}
                ...
                {isClass ? (
                    <Box
                        size={12}
                        className={`${isBaseLayer ? 'text-cyan-400' : 'text-emerald-400'} shrink-0`}
                    />
                ) : (
                    <CircleDot
                        size={12}
                        className={`${isBaseLayer ? 'text-cyan-400' : 'text-violet-400'} shrink-0`}
                    />
                )}

                {/* Name */}
                <span
                    className={`text-xs font-medium truncate flex-1 ${isBaseLayer ? 'text-muted-foreground' : ''}`}
                >
                    {node.name}
                </span>
...
function DetailPanel({
    ...
    const objectProps = relationships.filter(
        e => e.edge_type === 'object_property'
    );
    const dataProps = relationships.filter(
        e => e.edge_type === 'data_property'
    );
        ...
        <div className="h-full overflow-y-auto p-4 space-y-4">
            ...
            <div>
                <div className="flex items-center gap-2 mb-1">
                    {isClass ? (
                        <Box
                            size={16}
                            className={
                                layerName ? 'text-cyan-400' : 'text-emerald-400'
                            }
                        />
                    ) : (
                        <CircleDot
                            size={16}
                            className={
                                layerName ? 'text-cyan-400' : 'text-violet-400'
                            }
                        />
                    )}
                ...
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    <Badge
                        variant="outline"
                        className={`text-[10px] ${isClass ? 'text-emerald-400 border-emerald-500/30' : 'text-violet-400 border-violet-500/30'}`}
                    >
                        {isClass ? 'Class' : 'Instance'}
                    </Badge>
                    <Badge
                        variant="outline"
                        className={`text-[10px] ${node.status === 'approved' ? 'text-emerald-400 border-emerald-500/30' : 'text-amber-400 border-amber-500/30'}`}
                    >
                        {node.status}
                    </Badge>
                    {layerName && (
                        <Badge
                            variant="outline"
                            className="text-[10px] text-cyan-400 border-cyan-500/30 border-dashed"
                        >
            ...
            {node.base_item_uri && (
                <>
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-cyan-400/80 font-mono break-all">
                            {node.base_item_uri}
                        </p>
                    </div>
                </>
                ...
                <>
                    ...
                    <div>
                        <h4 className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider mb-1.5 flex items-center gap-1">
                            <ArrowRight size={11} /> Relationships (
                            {objectProps.length})
                        </h4>
                        <div className="space-y-1.5">
                            {objectProps.map(e => {
                                ...
                                return (
                                    <div
                                        key={e.id}
                                        className="flex items-center gap-2 text-xs bg-violet-500/5 rounded-md px-2.5 py-1.5 border border-violet-500/10"
                                    >
                    ...
                    <div>
                        ...
                        <div className="space-y-1.5">
                            {dataProps.map(e => (
                                <div
                                    key={e.id}
                                    className="flex items-center justify-between gap-2 text-xs bg-amber-500/5 rounded-md px-2.5 py-1.5 border border-amber-500/10"
                                >
                                    ...
                                    {e.description && !e.target_value && (
                                        <span className="text-foreground/60 truncate text-right">
                                            {e.description}
                                        </span>
                                    )}
                                </div>
...
export function OntologyGraph() {
    ...
    // Build hierarchy
    const tree = useMemo(
        () => buildTree(rawNodes, rawEdges),
        [rawNodes, rawEdges]
    );
    const { categories, orphans } = useMemo(
        () => deduplicateRoots(tree),
        [tree]
    );
    ...
    const activeCategory = activeCategoryId
        ? (nodeMap.get(activeCategoryId) ?? null)
        : null;
    ...
    const totalInstances = rawNodes.filter(
        n => n.node_type === 'individual'
    ).length;
    const totalEdges = rawEdges.filter(e => e.edge_type !== 'is_a').length;
    const avgConfidence =
        rawNodes.length > 0
            ? rawNodes.reduce(
                  (s: number, n: any) => s + (n.confidence || 0),
                  0
              ) / rawNodes.length
            : 0;
        ...
        return (
            <div className="flex-1 flex items-center justify-center text-muted-foreground">
                <div className="text-center space-y-2">
                    <Sparkles
                        size={32}
                        className="mx-auto text-emerald-500/40"
                    />
                    <p className="text-lg font-medium">No concepts yet</p>
                    <p className="text-sm">
                        Upload documents and run extraction to populate the
                        ontology
                    </p>
                </div>
            </div>
