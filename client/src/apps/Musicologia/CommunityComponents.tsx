            ...
            .then((d: Record<string, { count: number; reacted: boolean }>) => {
                if (!ignore) {
                    ...
                    for (const [emoji, info] of Object.entries(d)) {
                        if (info.count > 0)
                            newCounts.push({ emoji, count: info.count });
    ...
    const handleReact = async (emoji: string) => {
        ...
        if (res.ok) {
            ...
            setCounts(prev => {
                ...
                return Array.from(map.entries()).map(([e, c]) => ({
                    emoji: e,
                    count: c
                }));
            });
        ...
        return (
            <div className="flex gap-2 animate-pulse">
                {REACTION_EMOJIS.map(e => (
                    <div key={e} className="w-14 h-10 rounded-xl bg-muted/30" />
                ))}
            </div>
        );
    }
            ...
            {REACTION_EMOJIS.map(emoji => {
                ...
                return (
                    ...
                    >
                        ...
                        {count > 0 && (
                            <span
                                className={`text-xs font-mono ${active ? (dark ? 'text-purple-300' : 'text-purple-600') : textClass}`}
                            >
                                {count}
                            </span>
                        )}
            ...
            <div className="flex gap-3 group">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    >
                        ...
                        <p
                            className={`text-sm leading-relaxed ${textMain} opacity-80`}
                        >
                            {comment.body}
                        </p>
                    </div>
...
export function CommentThread({
    ...
    const load = useCallback(() => {
        let ignore = false;
        fetch(
            `/app/api/musicologia/comments?target_type=${targetType}&target_id=${targetId}`
        )
            .then(r => r.json())
    ...
    const handleDelete = async (id: number) => {
        const res = await fetch(`/app/api/musicologia/comments/${id}`, {
            method: 'DELETE'
        });
            ...
            {currentUserId ? (
                <div className="flex gap-3">
                    ...
                    <div className="flex-1 space-y-2">
                        {replyTo && (
                            <div
                                className={`flex items-center justify-between text-xs ${textMuted} px-2`}
                            >
                                <span>
                                    Replying to{' '}
                        ...
                        <Textarea
                            ...
                            style={
                                dark ? { background: inputBg, borderColor } : {}
                            }
                            onKeyDown={e => {
                                if (
                                    e.key === 'Enter' &&
