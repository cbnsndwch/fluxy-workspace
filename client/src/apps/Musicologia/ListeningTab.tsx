...
export function ScrobblerStatus({
    spotifyConnected
}: {
...
function QueueSection() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                '/app/api/musicologia/staging?status=pending'
            );
            const data = (await res.json()) as { staging: StagingItem[] };
            // Also load importing ones
            const res2 = await fetch(
                '/app/api/musicologia/staging?status=importing'
            );
    ...
    const handleImport = async (item: StagingItem) => {
        ...
        try {
            await fetch(`/app/api/musicologia/staging/${item.id}/import`, {
                method: 'POST'
            });
            await load();
        } catch {
    ...
    const handleDismiss = async (item: StagingItem) => {
        try {
            await fetch(`/app/api/musicologia/staging/${item.id}/dismiss`, {
                method: 'POST'
            });
    ...
    if (items.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-48 gap-3 text-muted-foreground">
                ...
                <p className="text-sm">
                    Nothing in the queue yet — start listening on Spotify
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-2">
            {items.map(item => {
                const isImporting =
                    importing.has(item.id) || item.status === 'importing';
                return (
                    <div
                        key={item.id}
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="text-[10px] text-muted-foreground/60">
                                    first heard{' '}
                                    {relativeTime(item.first_played_at)}
                                </span>
                            </div>
                        </div>
...
function HistorySection() {
    ...
    const load = useCallback(async (off: number, append = false) => {
        ...
        try {
            const res = await fetch(
                `/app/api/musicologia/plays?limit=${LIMIT}&offset=${off}`
            );
