    ...
    const handleSaveLrc = async () => {
        ...
        try {
            const res = await fetch(
                `/app/api/musicologia/tracks/${trackId}/lyrics`,
                {
                    method: 'POST',
                    ...
                    {lines.map((line, i) => {
                        ...
                        return (
                            <button
                                ...
                                onClick={() =>
                                    setCurrentTime(line.time_seconds)
                                }
                            >
                                {line.text}
                            </button>
            ...
            <div className="border-t border-white/10 pt-4">
                ...
                {showLrcAdmin && (
                    <div className="mt-4 flex flex-col gap-3">
                        ...
                        {lrcPreview.length > 0 && (
                            <div className="rounded-lg bg-white/5 border border-white/10 p-3 max-h-40 overflow-y-auto">
                                <p className="text-[10px] text-white/40 uppercase tracking-wider mb-2">
                                    Preview ({lrcPreview.length} lines)
                                </p>
                                {lrcPreview.slice(0, 10).map((l, i) => (
                                    ...
                                    >
                                        [{formatTime(l.time_seconds)}] {l.text}
                                    </p>
                                ))}
                                {lrcPreview.length > 10 && (
                                    <p className="text-xs text-white/30 mt-1">
                                        +{lrcPreview.length - 10} more…
                                    </p>
                                )}
                            </div>
                        )}
                        ...
                        {saveMsg && (
                            <p className="text-xs text-white/50">{saveMsg}</p>
                        )}
                    </div>
                )}
            </div>
...
export default function ImmersivePlayer() {
        ...
        fetch(`/app/api/musicologia/tracks/${artistSlug}/${trackSlug}`)
            .then(r => {
                ...
                return r.json() as Promise<
                    TrackDetail & { lrcLyrics: LyricLine[] }
                >;
            })
            .then(data => {
                if (!ignore && data) {
    ...
    useEffect(() => {
        ...
        fetch(`/app/api/musicologia/tracks/${trackId}/reactions`)
            ...
            .then(data => {
                if (!ignore && data) setReactions(data);
            })
            .catch(() => {});

        fetch(
            `/app/api/musicologia/comments?target_type=track&target_id=${trackId}`
        )
        ...
        async (emoji: string) => {
            ...
            setReactions(prev => {
                ...
                for (const e of REACTION_EMOJIS) {
                    ...
                    if (e === emoji) {
                        updated[e] = {
                            count: cur.count + (toggling_off ? -1 : 1),
                            reacted: !cur.reacted
                        };
                    } else if (cur.reacted) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (fresh.ok)
                        setReactions((await fresh.json()) as Reactions);
                }
            } catch {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            if (taglineRef.current) {
                ...
                const text =
                    taglineRef.current.querySelector('.hero-tagline-text');
                if (rule) {
            ...
            if (glowRef.current)
                exitTl.to(
                    glowRef.current,
                    { scale: 1.3, opacity: 0.3, duration: 1 },
                    0
                );
            ...
            dnaBarRefs.current.forEach((bar, i) => {
                ...
                const featureKeys: Array<keyof TrackDNA> = [
                    'energy',
                    'valence',
                    'danceability',
                    'acousticness',
    ...
    const credits = parseJSON<Array<{ role: string; name: string }>>(
        lore?.credits,
        []
    );
        ...
        >
            ...
            >
                ...
                <div
                    ...
                    style={{
                        animation: 'gn-glow-breathe 8s ease-in-out infinite'
                    }}
                >
                    <div
                ...
                <div className="relative z-10 flex flex-col items-center px-6 max-w-5xl w-full">
                    ...
                    {lore?.tagline && (
                        ...
                        >
                            ...
                            />
                            <p
                                className="hero-tagline-text text-sm md:text-base tracking-widest uppercase text-center"
                                style={{
                                    color: palette[1] ?? palette[0] ?? 'white'
                                }}
                            >
                                {lore.tagline}
                            </p>
                            ...
                            <div ref={storyRef} className="mt-8 max-w-2xl">
                                {lore.story
                                    ...
                                    .map((p, i) => (
                                        <p
                                            key={i}
                                            className="text-white/60 leading-relaxed text-base mb-4 last:mb-0"
                                        >
                                            {p}
                                        </p>
                                    ))}
            ...
            <section className="relative z-10 py-16 px-6">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div
                        className="mt-8 rounded-2xl bg-white/3 border border-white/8 p-6"
                        style={{
                            ...
                            {REACTION_EMOJIS.map(emoji => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <span
                                            className={`text-xs font-mono tabular-nums ${entry.reacted ? 'text-white/80' : 'text-white/40'}`}
                                        >
                                            {entry.count}
                                        </span>
                                    </button>
