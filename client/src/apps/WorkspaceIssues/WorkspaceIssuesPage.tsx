...
function IssueRow({
    ...
    const agent = issue.agent_status
        ? AGENT_STATUS_META[issue.agent_status]
        : null;

    return (
        <div
            // oxlint-disable-next-line jsx_a11y/prefer-tag-over-role -- nested checkbox+delete buttons
            role="button"
            tabIndex={0}
            className={cn(
                    ...
                    >
                        {attachments.map(url => (
                            <button
                                type="button"
                                key={url}
                                className="h-7 w-7 rounded border border-border cursor-zoom-in overflow-hidden p-0"
...
function KanbanCard({
    ...
    return (
        <div
            // oxlint-disable-next-line jsx_a11y/prefer-tag-over-role -- nested delete button
            role="button"
            tabIndex={0}
            ...
            <div className="mt-1 space-y-2">
                ...
                <div>
                    <p className="text-sm font-medium leading-snug">
                        <span className="text-muted-foreground/60 font-normal mr-1">
                            #{issue.id}
                        </span>
                        {issue.title}
                    </p>
...
function DispatchBatchesPanel() {
    ...
    const active = batches.filter(
        b => b.status === 'working' || b.status === 'syncing'
    );
    ...
    const handleDragStart = (e: React.MouseEvent | React.TouchEvent) => {
        ...
        const onMove = (clientY: number) => {
            ...
            const maxH = Math.round(rect.height * 0.9);
        ...
        >
            ...
            >
                ...
                <span className="text-xs font-medium text-muted-foreground flex-1">
                    Agent Dispatch
                    {active.length > 0 && (
                        <span className="ml-1.5 text-amber-500 animate-pulse">
...
export default function WorkspaceIssuesPage() {
    ...
    const [search, setSearch] = useState(
        () => localStorage.getItem('issues_search') ?? ''
    );
    const [filterStatus, setFilterStatus] = useState<Status | 'all'>(
        () =>
            (localStorage.getItem('issues_filterStatus') as Status | 'all') ??
            'all'
    );
    const [filterPriority, setFilterPriority] = useState<Priority | 'all'>(
        () =>
            (localStorage.getItem('issues_filterPriority') as
                | Priority
                | 'all') ?? 'all'
    );
    const [filterCategory, setFilterCategory] = useState<Category | 'all'>(
        () =>
            (localStorage.getItem('issues_filterCategory') as
                | Category
                | 'all') ?? 'all'
    );
    const [filterApp, setFilterApp] = useState<AppTarget | 'all'>(
        () =>
            (localStorage.getItem('issues_filterApp') as AppTarget | 'all') ??
            'all'
    );
    ...
    const [viewMode, setViewMode] = useState<'list' | 'kanban'>(
        () =>
            (localStorage.getItem('issues_viewMode') as 'list' | 'kanban') ??
            'list'
    );
    ...
    const [modal, setModal] = useState<
        'new' | (Partial<Issue> & { _defaultStatus?: Status }) | null
    >(null);
    ...
    const filtered = issues
        .filter(i => {
            ...
            if (
                filterAgentActive &&
                i.agent_status !== 'queued' &&
                i.agent_status !== 'working'
            )
                return false;
            if (
                search &&
                ...
                <>
                    {selectMode && selectedIds.size > 0 && (
                        ...
                        >
                            ...
                            {dispatching
                                ? 'Dispatching…'
                                : `Dispatch ${selectedIds.size} to Sebastian`}
                        </Button>
                    )}
                    <Button
                        size="sm"
            ...
            <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
                ...
                <div className="flex items-center gap-2 px-6 py-2.5 border-b border-border/50 shrink-0 flex-wrap">
                    ...
                    <Select
                        value={filterApp}
                        onValueChange={v =>
                            persistFilterApp(v as AppTarget | 'all')
                        }
                    >
                            ...
                            <TooltipTrigger asChild>
                                ...
                                >
                                    <BotMessageSquare
                                        className={cn(
                                            'h-3.5 w-3.5',
                                            filterAgentActive && 'animate-pulse'
                                        )}
                                    />
                                    Agent active
                                </Button>
