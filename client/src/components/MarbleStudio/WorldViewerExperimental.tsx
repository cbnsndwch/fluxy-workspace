...
function viewerSrc(world: MarbleWorld): string {
  ...
  return `/app/api/marble-studio/viewer?assets=${encoded}&mode=experimental`;
}
...
// ── Experimental World Viewer ─────────────────────────────────────────────────
// Uses the enhanced viewer with Marble-style loading effect:
// - Canvas-based particle wave overlay during initial load
// - Slower front-to-back depth reveal (camera.far sweep synced with wave particles)
// - Ring highlight at the quality-upgrade crossfade boundary
// - Particles oscillate in coordinated waves and fade as real splats appear

export default function WorldViewerExperimental({ world, onDelete, children }: Props) {
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {world.error_msg || "An unknown error occurred"}
          </p>
        </div>
        <Button
          variant="outline"
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* 3D Canvas — experimental pipeline */}
      ...
      {/* Metadata footer */}
      <div id="metadata-footer" className="border-t border-border/50 bg-card px-5 py-3">
        <div className="flex items-start gap-6">
          <div className="flex-1 min-w-0 space-y-0.5">
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant="outline"
                className="text-[10px] py-0 border-green-500/30 text-green-500 shrink-0"
              >
                {world.model}
              </Badge>
            </div>
            ...
            <p className="text-xs text-muted-foreground/50 italic line-clamp-1">"{world.prompt}"</p>
          </div>
