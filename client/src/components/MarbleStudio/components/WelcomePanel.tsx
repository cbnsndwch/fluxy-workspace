...
export function WelcomePanel({ onNew }: WelcomePanelProps) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-6 px-8 text-center">
      ...
      <div>
        <h2 className="text-xl font-semibold">World Labs Marble Studio</h2>
        <p className="text-sm text-muted-foreground mt-2 max-w-sm leading-relaxed">
          Generate explorable 3D Gaussian Splat worlds from text, images, video, or multi-view
          captures using World Labs Marble AI.
        </p>
      </div>
      <div className="grid grid-cols-3 gap-3 text-left max-w-lg w-full">
        {[
          ...
          <div key={title} className="rounded-xl border border-border/50 bg-card p-4 space-y-1.5">
