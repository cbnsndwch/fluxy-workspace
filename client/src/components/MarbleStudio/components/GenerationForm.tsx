...
import { Sparkles, Shuffle, Plus, Trash2, AlertCircle, Loader2 } from "lucide-react";
...
export function GenerationForm({ onGenerated }: GenerationFormProps) {
  ...
  const updateSlot = (id: string, field: keyof Omit<ImageSlot, "id">, value: string) => {
      ...
      <div className="max-w-2xl mx-auto px-8 py-10">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-lg font-semibold">Generate a New World</h2>
          </div>
          <p className="text-sm text-muted-foreground">
            Describe a place and Marble AI will generate an explorable 3D Gaussian Splat world.
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Tabs value={mode} onValueChange={(v) => setMode(v as PromptMode)}>
            <TabsList className="w-full">
              ...
              <TabsTrigger value="multi-image" className="flex-1 text-xs">
                Multi-Image
              </TabsTrigger>
            ...
            <TabsContent value="text" className="space-y-3 pt-4">
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <Label htmlFor="text-prompt">World Description</Label>
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>Random prompt</TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
                <Textarea
                ...
                <p className="text-xs text-muted-foreground">
                  Be spatial and specific — describe materials, lighting, architecture, and
                  atmosphere.
                </p>
              </div>
            </TabsContent>
            ...
            <TabsContent value="image" className="space-y-4 pt-4">
              <div className="space-y-1.5">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  A publicly accessible JPEG, PNG, or WebP. High-resolution panoramas and wide-angle
                  shots work best.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 rounded-lg border border-border/50 bg-muted/30 px-4 py-3">
                <div className="flex-1">
                  <p className="text-sm font-medium">Panoramic Image</p>
                  <p className="text-xs text-muted-foreground">
                    Enable if your image is a 360° equirectangular panorama
                  </p>
                </div>
                <Switch checked={isPano} onCheckedChange={setIsPano} />
              </div>
            ...
            {/* ── Multi-Image ───────────────────────────────────── */}
            <TabsContent value="multi-image" className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Multiple views of the same scene. The azimuth angle (0–360°) indicates the
                  shooting direction — 0° front, 90° right, 180° back.
                </p>
                <div className="space-y-2">
                  {slots.map((slot, i) => (
                    <div
                    ...
                    >
                      <div className="h-6 w-6 rounded-md bg-green-500/10 flex items-center justify-center shrink-0">
                        <span className="text-[10px] font-semibold text-green-500">{i + 1}</span>
                      </div>
                      <Input
                        value={slot.url}
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Input
                          ...
                          max="360"
                          value={slot.azimuth}
                          onChange={(e) => updateSlot(slot.id, "azimuth", e.target.value)}
            ...
            <TabsContent value="video" className="space-y-4 pt-4">
              <div className="space-y-1.5">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  A publicly accessible MP4, MOV, or MKV. Walking tours and smooth panning footage
                  work best.
                </p>
              </div>
              <div className="space-y-1.5">
            ...
            <TabsContent value="presets" className="pt-4">
              <div className="space-y-5">
                ...
                <p className="text-xs text-muted-foreground/50 text-center pt-1">
                  Click a preset to load it into the Text editor
                </p>
              </div>
            </TabsContent>
          </Tabs>
