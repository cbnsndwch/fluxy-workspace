...
function RowModal({
  ...
  const editableCols =
    columns.filter((c) => (!c.pk || isNew ? false : false)).length === 0
      ? columns.filter((c) => c.name !== "id" && c.name !== "created_at" && c.name !== "updated_at")
      : columns.filter(
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isNew ? `New row in ${table}` : `Edit row`}</DialogTitle>
        </DialogHeader>
          ...
          {editableCols.map((col) => (
            <div key={col.name} className="space-y-1">
              <label className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
                {col.name}
                <span className="text-[10px] text-muted-foreground/60 font-normal">{col.type}</span>
                {col.notnull ? <span className="text-red-500 text-[10px]">required</span> : null}
              </label>
              <Input
                value={form[col.name] ?? ""}
                onChange={(e) =>
                ...
                placeholder={col.dflt_value ? `default: ${col.dflt_value}` : undefined}
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-sm">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          <code className="text-xs bg-muted px-1 py-0.5 rounded">{table}</code> will be permanently
          deleted.
        </p>
        {error && <p className="text-xs text-destructive">{error}</p>}
        <div className="flex justify-end gap-2 pt-2">
...
export default function DBViewerPage() {
  ...
  const loadTables = useCallback(async () => {
    ...
    if (data.length > 0 && !selectedTable)
      navigate(`/db-viewer/${data[0].name}`, { replace: true });
