...
import { CheckCircle2, CircleDot, Clock, ImagePlus, Mic, MicOff, X, XCircle } from 'lucide-react';
...
// ── Metadata ────────────────────────────────────────────────────────────────────
export const STATUS_META: Record<Status, { label: string; icon: ReactNode; color: string }> = {
...
export const AGENT_STATUS_META: Record<NonNullable<AgentStatus>, { label: string; color: string; pulse?: boolean }> = {
...
// ── Issue Modal ─────────────────────────────────────────────────────────────────
export function IssueModal({ issue, onClose, onSave }: {
    ...
    const handleSaveToDocs = async () => {
        ...
        try {
            await fetch(`/app/api/docs/file?path=${encodeURIComponent(docPath)}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <Dialog open onOpenChange={open => !open && onClose()}>
            <DialogContent className="sm:max-w-xl flex flex-col max-h-[90vh]" onPaste={handlePaste}>
                <DialogHeader>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1.5">
                                {!isNew && form.description.trim() && (
                                    ...
                                    >
                                        {savedToDocs ? '✓ Saved' : savingToDocs ? 'Saving…' : '↗ Save to Docs'}
                                    </button>
                                )}
                                {voiceDesc.supported && (
                                    <button
