      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button
            ...
            onClick={async () => {
              setDeleting(true);
              await fetch(`/app/api/issues/${issue.id}`, {
                method: "DELETE",
              });
              onDeleted();
            }}
  ...
  return (
    ...
    >
      ...
      {selectMode ? (
        ...
        <div
          className={cn("w-1 self-stretch rounded-full shrink-0", {
      ...
      <div className="flex-1 min-w-0 py-0.5">
        <div className="flex items-baseline gap-2 flex-wrap">
          <span className="text-muted-foreground/60 font-normal text-[11px] shrink-0">
            #{issue.id}
          </span>
          ...
          {agent && (
            ...
            >
              {issue.agent_status === "working" ? (
                <Loader2 className="h-2.5 w-2.5 animate-spin" />
              ) : (
                <BotMessageSquare className="h-2.5 w-2.5" />
              )}
...
function KanbanCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn("absolute top-0 left-3 right-3 h-0.5 rounded-full", {
          "bg-red-500": issue.priority === "critical",
          "bg-orange-500": issue.priority === "high",
          "bg-sky-500": issue.priority === "medium",
      ...
      <div className="mt-1 space-y-2">
        ...
        {attachments.length > 0 && (
          <div
            className="relative rounded-md overflow-hidden border border-border/60 -mx-0.5"
            onClick={(e) => e.stopPropagation()}
          >
            <img src={attachments[0]} alt="" className="w-full object-cover max-h-48" />
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-5 pb-3 space-y-2">
          {batches.map((batch) => {
            ...
            return (
              <div
                key={batch.id}
                className="rounded-lg border border-border/60 bg-muted/10 p-2.5 text-xs space-y-1.5"
              >
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={cn(
                      "font-medium",
                      sm.color,
                      batch.status === "working" && "animate-pulse",
                    )}
                  >
                    {sm.label}
                  </span>
                ...
                {batch.sync_report && (
                  <details className="mt-1">
                    ...
                    <pre className="mt-1 text-[10px] whitespace-pre-wrap text-muted-foreground/70 leading-relaxed">
                      {batch.sync_report}
                    </pre>
                  </details>
                )}
...
export default function WorkspaceIssuesPage() {
  ...
  const { data: issues = [], isLoading: loading } = useQuery<Issue[]>({
    ...
    refetchInterval: (query) => {
      ...
      const hasActive = data?.some(
        (i) => i.agent_status === "queued" || i.agent_status === "working",
      );
  ...
  const [filterStatus, setFilterStatus] = useState<Status | "all">(
    () => (localStorage.getItem("issues_filterStatus") as Status | "all") ?? "all",
  );
  const [filterPriority, setFilterPriority] = useState<Priority | "all">(
    () => (localStorage.getItem("issues_filterPriority") as Priority | "all") ?? "all",
  );
  const [filterCategory, setFilterCategory] = useState<Category | "all">(
    () => (localStorage.getItem("issues_filterCategory") as Category | "all") ?? "all",
  );
  const [filterApp, setFilterApp] = useState<AppTarget | "all">(
    () => (localStorage.getItem("issues_filterApp") as AppTarget | "all") ?? "all",
  );
  const [filterAgentActive, setFilterAgentActive] = useState(
    () => localStorage.getItem("issues_filterAgentActive") === "true",
  );
  const [viewMode, setViewMode] = useState<"list" | "kanban">(
    () => (localStorage.getItem("issues_viewMode") as "list" | "kanban") ?? "list",
  );

  // Write state to localStorage in the event handlers — no useEffect needed
  const persistSearch = (v: string) => {
    setSearch(v);
    localStorage.setItem("issues_search", v);
  };
  const persistFilterStatus = (v: Status | "all") => {
    setFilterStatus(v);
    localStorage.setItem("issues_filterStatus", v);
  };
  const persistFilterPriority = (v: Priority | "all") => {
    setFilterPriority(v);
    localStorage.setItem("issues_filterPriority", v);
  };
  const persistFilterCategory = (v: Category | "all") => {
    setFilterCategory(v);
    localStorage.setItem("issues_filterCategory", v);
  };
  const persistFilterApp = (v: AppTarget | "all") => {
    setFilterApp(v);
    localStorage.setItem("issues_filterApp", v);
  };
  const toggleFilterAgentActive = () =>
    setFilterAgentActive((v) => {
      const next = !v;
      localStorage.setItem("issues_filterAgentActive", String(next));
      return next;
    });
  const persistViewMode = (v: "list" | "kanban") => {
    setViewMode(v);
    localStorage.setItem("issues_viewMode", v);
  };

  const [modal, setModal] = useState<"new" | (Partial<Issue> & { _defaultStatus?: Status }) | null>(
    null,
  );
  ...
  const openNewWithStatus = (status: Status) =>
    setModal({ _defaultStatus: status } as Partial<Issue> & {
  ...
  const dropMutation = useMutation({
    ...
    onMutate: ({ issueId, newStatus }) => {
      ...
      qc.setQueryData<Issue[]>(
        ["issues"],
        (old) => old?.map((i) => (i.id === issueId ? { ...i, status: newStatus } : i)) ?? [],
      );
      return { prev };
    },
  ...
  const filtered = issues
    .filter((i) => {
      ...
      if (filterAgentActive && i.agent_status !== "queued" && i.agent_status !== "working")
        return false;
      if (
        search &&
  ...
  const grouped = STATUS_ORDER.reduce<Record<Status, Issue[]>>(
    (acc, status) => {
      ...
      actions={
        <>
          ...
          <div className="flex items-center border border-border rounded-md overflow-hidden">
            <button
              className={cn(
                "px-2 py-1.5 transition-colors cursor-pointer",
                viewMode === "list"
            ...
            <button
              className={cn(
                "px-2 py-1.5 transition-colors border-l border-border cursor-pointer",
                viewMode === "kanban"
                  ? "bg-muted text-foreground"
                  : "text-muted-foreground hover:text-foreground hover:bg-muted/50",
              )}
      ...
      <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className={cn(
                    "h-8 gap-1.5 text-xs",
                    filterAgentActive && "bg-violet-500/10 text-violet-500 border-violet-500/30",
                  )}
                >
                  <BotMessageSquare
                    className={cn("h-3.5 w-3.5", filterAgentActive && "animate-pulse")}
                  />
                  Agent active
                </Button>
              </TooltipTrigger>
              <TooltipContent side="bottom" className="text-xs">
                Show only issues being worked by an agent
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {modal && (
          <IssueModal
            ...
            onClose={() => {
              setModal(null);
              if (issueIdParam) navigate("/issues", { replace: true });
            }}
            onSave={() => {
              setModal(null);
              qc.invalidateQueries({ queryKey: ["issues"] });
