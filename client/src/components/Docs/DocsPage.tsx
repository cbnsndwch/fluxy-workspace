    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-sm">
        ...
        <p className="text-sm text-muted-foreground">
          <code className="text-xs bg-muted px-1 py-0.5 rounded">{name}</code> will be permanently
          deleted.
        </p>
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button
            ...
            onClick={async () => {
              setDeleting(true);
              await fetch(`/app/api/docs/file?path=${encodeURIComponent(path)}`, {
                method: "DELETE",
              });
              onDeleted();
            }}
                ...
                <div className="flex items-center gap-1.5">
                  {editing ? (
                    ...
                    <>
                      ...
                      <button
                        onClick={() => {
                          const fp = findBySlug(tree, selectedPath!);
