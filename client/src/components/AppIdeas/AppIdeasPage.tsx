...
import { Plus, Filter, Lightbulb, RefreshCw, GitBranch, ZoomIn, ZoomOut, Maximize2, Lock, Unlock } from 'lucide-react';
import { AppLayout } from '@/components/ui/app-layout';
import { IdeaCardNode } from './IdeaCardNode';
import IdeaModal from './IdeaModal';
import { STAGE_META, GROUP_COLORS } from './types';
...
function StringEdge({ id, sourceX, sourceY, targetX, targetY, label, markerEnd, style }: EdgeProps) {
    ...
    return (
        <>
            <path id={id} d={d} fill="none" style={style} markerEnd={markerEnd as string} />
            {label && (
                <g transform={`translate(${midX},${midY})`}>
                    <rect x="-22" y="-8" width="44" height="16" rx="3" fill="rgba(0,0,0,0.7)" opacity="0.9" />
                    <text textAnchor="middle" dominantBaseline="middle" style={{ fontSize: 9, fill: '#a0a0c0', fontStyle: 'italic', fontFamily: 'serif' }}>
                        {label as string}
                    </text>
                </g>
            )}
...
function layoutIdeas(ideas: AppIdea[]): Record<number, { x: number; y: number }> {
...
function CanvasControls({ locked, setLocked }: { locked: boolean; setLocked: (v: boolean) => void }) {
    const { zoomIn, zoomOut, fitView } = useReactFlow();

    const btn = 'cursor-pointer flex items-center justify-center w-8 h-8 rounded-md text-foreground hover:bg-muted transition-colors';
            ...
            <div className="flex flex-col gap-0.5 p-1 rounded-xl bg-card border border-border shadow-md">
                ...
                <Tooltip>
                    <TooltipTrigger asChild>
                        ...
                        >
                            {locked ? <Lock size={14} strokeWidth={2} /> : <Unlock size={14} strokeWidth={2} />}
                        </button>
                    </TooltipTrigger>
                    <TooltipContent side="right">{locked ? 'Unlock canvas' : 'Lock canvas'}</TooltipContent>
                </Tooltip>
            </div>
        </TooltipProvider>
    );
...
export default function AppIdeasPage() {
    ...
    const groups = useMemo(() => [...new Set(ideas.map(i => i.group_name).filter(Boolean) as string[])], [ideas]);

    // Group to palette index map
    const groupPaletteMap = useMemo(() => {
        const map: Record<string, number> = {};
        const gs = [...new Set(ideas.map(i => i.group_name).filter(Boolean) as string[])];
        gs.forEach((g, i) => { map[g] = i % PAPER_PALETTES.length; });
        return map;
    }, [ideas]);

    // Build React Flow nodes
    const buildNodes = useCallback((ideasList: AppIdea[], stageF: Stage | 'all', groupF: string | 'all') => {
        const positions = layoutIdeas(ideasList);
        return ideasList.map(idea => {
            const paletteIdx = idea.group_name !== null && idea.group_name !== undefined ? (groupPaletteMap[idea.group_name] ?? 0) : 0;
            ...
            return {
                ...
                data: {
                    ...
                    color: idea.group_name ? groupColorMap[idea.group_name] : null,
                    rotation: seedRotation(idea.id),
                    paperColor: palette.bg,
                    paperBorder: palette.border,
                    stackColor: palette.stack,
                    isFiltered: (stageF !== 'all' && idea.stage !== stageF) || (groupF !== 'all' && idea.group_name !== groupF),
                    onEdit: (i: AppIdea) => { setEditingIdea(i); setShowModal(true); },
                    onDelete: async (id: number) => {
                        await fetch(`/app/api/app-ideas/${id}`, { method: 'DELETE' });
    ...
    const buildEdges = useCallback((conns: AppIdeaConnection[]) => {
        return conns.map(c => ({
            ...
            type: 'string',
            animated: false,
            style: { stroke: '#7c6fcd', strokeWidth: 1.5, opacity: 0.55, strokeDasharray: '5 4' },
            markerEnd: { type: MarkerType.Arrow, width: 12, height: 12, color: '#7c6fcd' },
        } as Edge));
    }, []);
    ...
    const handleNodesChange = useCallback((changes: NodeChange[]) => {
        ...
        const moves = changes.filter(c => c.type === 'position' && c.dragging === false);
        ...
        saveTimer.current = setTimeout(() => {
            moves.forEach(c => {
                ...
                fetch(`/app/api/app-ideas/${c.id}/position`, {
                    ...
                    body: JSON.stringify({ pos_x: c.position.x, pos_y: c.position.y }),
                }).catch(console.error);
            });
        }, 400);
    }, [onNodesChange]);
    ...
    const onConnect = useCallback(async (params: Connection) => {
        const r = await fetch('/app/api/app-ideas/connections', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ source_id: Number(params.source), target_id: Number(params.target) }),
        });
        const conn = await r.json();
        setConnections(prev => [...prev, conn]);
        setEdges(eds => addEdge({ ...params, type: 'string', style: { stroke: '#7c6fcd', strokeWidth: 1.5, opacity: 0.55, strokeDasharray: '5 4' } }, eds));
    }, [setEdges]);
    ...
    const handleSave = async (data: Partial<AppIdea>) => {
        if (editingIdea) {
            ...
            setIdeas(prev => prev.map(i => i.id === updated.id ? { ...updated, tags: typeof updated.tags === 'string' ? JSON.parse(updated.tags) : updated.tags } : i));
        } else {
            const pos = { pos_x: Math.random() * 400, pos_y: Math.random() * 400 };
            const r = await fetch('/app/api/app-ideas', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            setIdeas(prev => [...prev, { ...created, tags: typeof created.tags === 'string' ? JSON.parse(created.tags) : created.tags }]);
        }
        setShowModal(false);
        setEditingIdea(null);
    };
    ...
    const headerActions = (
        <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-1.5">
                <button
                    ...
                    className={`cursor-pointer px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
                        stageFilter === 'all' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-accent'
                    }`}
                >
                    All
                </button>
                    ...
                    return (
                        <button
                            ...
                            className={`cursor-pointer flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border transition-all ${
                                stageFilter === s ? `${m.bg} ${m.color} ${m.border}` : 'border-transparent text-muted-foreground hover:bg-accent'
                            }`}
                        >
            ...
            <Tooltip>
                ...
                <TooltipContent side="bottom">{showEdges ? 'Hide connections' : 'Show connections'}</TooltipContent>
            </Tooltip>
        ...
        >
            ...
            {showFilters && (
                <div className="flex items-center gap-1.5 px-4 py-2 overflow-x-auto shrink-0 bg-background/90 border-b border-border">
                    <button onClick={() => setStageFilter('all')} className={`cursor-pointer whitespace-nowrap px-2.5 py-1 rounded-full text-xs font-medium transition-all ${stageFilter === 'all' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-accent'}`}>
                        All stages
                    </button>
                    {(Object.keys(STAGE_META) as Stage[]).map(s => {
                        const m = STAGE_META[s];
            ...
            {groups.length > 0 && (
                <div className="flex items-center gap-1.5 px-4 py-1.5 overflow-x-auto shrink-0 bg-background/60 border-b border-border/50">
                    <span className="text-xs text-muted-foreground/50 shrink-0">Group:</span>
                    ...
                    {groups.map(g => (
                        <button key={g} onClick={() => setGroupFilter(groupFilter === g ? 'all' : g)}
                            ...
                            <span className="w-2 h-2 rounded-full shrink-0" style={{ background: groupColorMap[g] }} />
                            {g}
                        </button>
                    ))}
                </div>
            ...
            <div className="flex-1 relative">
                {loading ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-center">
                        ...
                        <div>
                            <p className="font-semibold text-base mb-1">No ideas yet</p>
                            <p className="text-sm text-muted-foreground mb-4">Start capturing your app ideas on the canvas</p>
                            <button
                    ...
                    >
                        ...
                        <Panel position="bottom-left">
                            <CanvasControls locked={locked} setLocked={setLocked} />
                        </Panel>
                        <MiniMap
                            style={{
                            ...
                            nodeColor={n => {
                                ...
                                const paletteIdx = idea.group_name ? (groupPaletteMap[idea.group_name] ?? 0) : 0;
                                return PAPER_PALETTES[paletteIdx].border;
                            }}
                            maskColor="rgba(0,0,0,0.5)"
                        />
                        <Panel position="bottom-center">
                            <div className="text-xs text-muted-foreground/40 pb-2 italic select-none tracking-wide">
                                drag cards · shift+drag to select many · draw connections between handles
                            </div>
                        </Panel>
                    </ReactFlow>
                )}
