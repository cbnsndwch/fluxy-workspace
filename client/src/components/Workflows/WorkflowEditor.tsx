      ...
      >
        ...
        <Panel position="bottom-left">
          <div className="flex flex-col gap-0.5 p-1 rounded-xl bg-card border border-border shadow-md">
            ...
            <button className={btn} onClick={() => fitView({ duration: 300, padding: 0.2 })}>
              <Maximize2 size={12} />
            </button>
          </div>
        </Panel>
  ...
  useEffect(() => {
    fetch(`/app/api/workflows/${workflowId}`)
      ...
      .then((wf: Workflow) => {
        ...
        setEdges(
          (JSON.parse(wf.edges || "[]") as Edge[]).map(({ animated: _a, ...e }) => ({
            ...e,
            style: { stroke: "#94a3b8", strokeWidth: 1.5 },
            markerEnd: { type: MarkerType.ArrowClosed, width: 12, height: 12, color: "#94a3b8" },
          })),
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex flex-1 min-h-0">
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              />
              {showRunner && <WorkflowRunner run={lastRun} nodes={runNodes} isRunning={running} />}
            </div>
