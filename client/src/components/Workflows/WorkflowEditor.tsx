...
import { ArrowLeft, Play, Loader2, ZoomIn, ZoomOut, Maximize2, Trash2, Download, Copy } from 'lucide-react';
...
import type { Workflow, WorkflowNodeType, WorkflowRun, WorkflowRunNode } from './types';
...
function EditorCanvas({
    ...
    const onDrop = useCallback((e: React.DragEvent) => {
        e.preventDefault();
        const type = e.dataTransfer.getData('workflow/nodeType') as WorkflowNodeType;
    ...
    const btn = 'cursor-pointer flex items-center justify-center w-7 h-7 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors';
            ...
            >
                ...
                <Panel position="bottom-left">
                    <div className="flex flex-col gap-0.5 p-1 rounded-xl bg-card border border-border shadow-md">
                        <button className={btn} onClick={() => zoomIn({ duration: 200 })}><ZoomIn size={13} /></button>
                        <button className={btn} onClick={() => zoomOut({ duration: 200 })}><ZoomOut size={13} /></button>
                        <div className="my-0.5 h-px bg-border mx-1" />
                        <button className={btn} onClick={() => fitView({ duration: 300, padding: 0.2 })}><Maximize2 size={12} /></button>
                    </div>
                </Panel>
                <MiniMap
                    style={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: 10 }}
                    maskColor="rgba(0,0,0,0.5)"
                />
                {nodes.length === 0 && (
                    <Panel position="top-center">
                        <div className="mt-16 text-center pointer-events-none select-none">
                            <p className="text-sm text-muted-foreground/40">Drag nodes from the left panel onto the canvas</p>
                        </div>
                    </Panel>
                )}
            </ReactFlow>
...
function EditorInner({ workflowId, onBack }: Props) {
    ...
    useEffect(() => {
        fetch(`/app/api/workflows/${workflowId}`)
            ...
            .then((wf: Workflow) => {
                ...
                setEdges((JSON.parse(wf.edges || '[]') as Edge[]).map(({ animated: _a, ...e }) => ({
                    ...e,
                    style: { stroke: '#94a3b8', strokeWidth: 1.5 },
                    markerEnd: { type: MarkerType.ArrowClosed, width: 12, height: 12, color: '#94a3b8' },
                })));
    ...
    const scheduleSave = useCallback(() => {
        ...
        saveTimer.current = setTimeout(() => {
            setNodes(nds => {
                setEdges(eds => {
                    const cleanEdges = eds.map(({ animated: _a, ...rest }) => rest);
                    fetch(`/app/api/workflows/${workflowId}`, {
                        method: 'PUT',
    ...
    const onDropNode = useCallback((type: WorkflowNodeType, position: { x: number; y: number }) => {
        const node: Node = {
            id: `${type}-${Date.now()}`,
            type,
            position,
    ...
    const saveName = useCallback(async () => {
        ...
        await fetch(`/app/api/workflows/${workflowId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: nameRef.current, nodes: workflow.nodes, edges: workflow.edges }),
        });
    ...
    const handleRun = useCallback(async () => {
        ...
        const es = new EventSource(`/app/api/workflows/${workflowId}/run/stream`);

        es.onmessage = (ev) => {
            try {
                const data = JSON.parse(ev.data);

                if (data.type === 'done' || data.type === 'run_complete' || data.type === 'run_error') {
                    es.close();
                    setRunning(false);
                    if (data.run_id) {
                        fetch(`/app/api/workflow-runs/${data.run_id}`)
                            .then(r => r.json())
                            .then(({ run, nodes: rn }: { run: WorkflowRun; nodes: WorkflowRunNode[] }) => {
                                setLastRun(run);
                                setRunNodes(rn);
                                const statusMap: Record<string, WorkflowRunNode> = {};
                                for (const n of rn) statusMap[n.node_id] = n;
                                setNodes(nds => nds.map(n => ({
                                    ...n,
                                    data: { ...n.data, runStatus: statusMap[n.id]?.status ?? undefined },
                                })));
                                ...
                                const executedNodes = new Set(
                                    rn.filter(n => n.status === 'success' || n.status === 'error').map(n => n.node_id)
                                );
                                setEdges(eds => eds.map(e => ({
                                    ...e,
                                    animated: executedNodes.has(e.target),
                                })));
                            })
                            .catch(console.error);
                    }
                    ...
                    // Live status update
                    setNodes(nds => nds.map(n =>
                        n.id === data.node_id ? { ...n, data: { ...n.data, runStatus: data.status } } : n
                    ));
                    setRunNodes(prev => {
                        ...
                        const updated: WorkflowRunNode = {
                            ...
                            node_type: nodes.find(n => n.id === data.node_id)?.type || '',
                            status: data.status,
                            input: null,
                        ...
                        if (idx >= 0) return prev.map((n, i) => i === idx ? updated : n);
                        return [...prev, updated];
                    });
                }
            } catch { /* ignore */ }
    ...
    const copySelected = useCallback(() => {
        setNodes(nds => {
            const selectedIds = new Set(
                nds.filter(n => n.selected || n.id === selectedNodeId).map(n => n.id)
            );
    ...
    const pasteClipboard = useCallback(() => {
        ...
        for (const n of cbNodes) {
            idMap[n.id] = `${n.type}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
        }
                ...
                <div className="flex items-center gap-2 px-3 py-2 border-b border-border bg-card/80 shrink-0">
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-7 w-7 cursor-pointer" onClick={onBack}>
                                <ArrowLeft size={15} />
                            </Button>
                        </TooltipTrigger>
                    ...
                    {selectedCount > 0 && (
                        <Tooltip>
                            ...
                            <TooltipContent>Copy selected ({selectedCount}) · Ctrl+C</TooltipContent>
                        </Tooltip>
                    )}
