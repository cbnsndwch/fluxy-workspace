...
function StatusBadge({ status }: { status: RunNodeStatus }) {
  ...
  return (
    <div className="absolute -top-2 -right-2 z-10">
      ...
      {status === "skipped" && <Minus size={16} className="text-muted-foreground" />}
    </div>
  );
}
...
export const BaseNode = memo(function BaseNode({
  ...
  return (
    ...
    >
      {runStatus && runStatus !== "pending" && <StatusBadge status={runStatus} />}
      ...
      {/* Header */}
      <div className={cn("flex items-center gap-2 px-3 py-2.5 rounded-t-md", bg)}>
      ...
      <div className="px-3 py-2">
        {children ??
          (preview ? (
            <p className="text-xs text-muted-foreground truncate font-mono">{preview}</p>
          ) : (
            <p className="text-xs text-muted-foreground/40 italic">no config</p>
          ))}
      </div>
