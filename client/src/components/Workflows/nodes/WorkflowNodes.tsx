  ...
  return (
    ...
    >
      <div className="space-y-1">
        ...
        {d.output_key && (
          <p className="text-[10px] text-muted-foreground/60">→ {String(d.output_key)}</p>
        )}
      </div>
    </BaseNode>
  );
...
export const CodeNode = memo(function CodeNode({ id, data, selected, isConnectable }: NodeProps) {
  const d = data as WFNodeData;
  const firstLine = String(d.code || "return input;")
    .split("\n")[0]
    .slice(0, 30);
  return (
    <BaseNode
...
export const DbQueryNode = memo(function DbQueryNode({
  id,
  data,
  selected,
  isConnectable,
  ...
  const d = data as WFNodeData;
  const preview = String(d.query || "SELECT …")
    .replace(/\s+/g, " ")
    .slice(0, 32);
  return (
    <BaseNode
