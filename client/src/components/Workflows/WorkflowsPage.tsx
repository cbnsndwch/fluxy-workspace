...
import { Plus, Play, Pencil, Trash2, Workflow, Clock, Upload, Copy } from 'lucide-react';
        ...
        <Dialog open={open} onOpenChange={v => !v && onClose()}>
            <DialogContent className="max-w-sm">
                ...
                <DialogFooter>
                    ...
                    <Button onClick={() => name.trim() && onSave(name, desc)} disabled={!name.trim()}>
                        {initial ? 'Save' : 'Create'}
                    </Button>
                </DialogFooter>
            </DialogContent>
...
export default function WorkflowsPage() {
    ...
    const updateMutation = useMutation({
        mutationFn: ({ wfId, name, description, nodes, edges }: { wfId: string; name: string; description: string; nodes: unknown; edges: unknown }) =>
    ...
    const handleDuplicate = useCallback(async (wf: WorkflowType) => {
    ...
    const handleImportFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const wfName = data.name || file.name.replace(/\.[^.]+$/, '') || 'Imported Workflow';
            const r = await fetch('/app/api/workflows', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name: wfName, description: data.description ?? null }),
            });
            ...
            alert('Failed to import workflow — make sure it is a valid JSON file.');
        }
    }, [navigate]);
        ...
        <AppLayout
            ...
            actions={
                <>
                    ...
                    <Button size="sm" className="gap-1.5 cursor-pointer" onClick={() => { setEditing(null); setDialogOpen(true); }}>
                        <Plus size={14} />
                        New workflow
                    </Button>
                </>
            ...
            <div className="overflow-y-auto h-full p-5">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-48 gap-4 text-center">
                        ...
                        <div>
                            <p className="font-semibold text-sm mb-1">No workflows yet</p>
                            <p className="text-xs text-muted-foreground mb-4">Build your first automated workflow</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-w-4xl">
                        {workflows.map(w => (
                            <WorkflowCard
                                ...
                                onEdit={wf => { setEditing(wf); setDialogOpen(true); }}
