import { CheckCircle2, XCircle, Loader2, Minus, ChevronDown, ChevronRight, Clock } from 'lucide-react';
...
function StatusIcon({ status }: { status: RunNodeStatus | 'running' | 'success' | 'error' }) {
    if (status === 'running') return <Loader2 size={13} className="animate-spin text-blue-400 shrink-0" />;
    ...
    return (
        <div className="border-b border-border/50 last:border-0">
            ...
            >
                ...
                <span className="text-xs text-foreground flex-1 truncate font-mono">{node.node_id.slice(0, 8)}…</span>
                <span className={cn(
                ...
                {node.duration_ms != null && (
                    <span className="flex items-center gap-0.5 text-[10px] text-muted-foreground/60">
                        <Clock size={9} />{node.duration_ms}ms
                    </span>
                )}
                {(hasOutput || hasError) && (
            ...
            {expanded && (
                <div className="px-3 pb-2">
                    {hasError && (
                        <pre className="text-[10px] text-red-400 bg-red-500/10 rounded p-2 overflow-x-auto whitespace-pre-wrap">{node.error}</pre>
                    )}
                    {hasOutput && !hasError && (
                        <pre className="text-[10px] text-muted-foreground bg-muted/30 rounded p-2 overflow-x-auto whitespace-pre-wrap max-h-48">
                            {(() => { try { return JSON.stringify(JSON.parse(node.output!), null, 2); } catch { return node.output; } })()}
                        </pre>
                    )}
                </div>
            )}
