...
const CRON_EXAMPLES = '*/5 * * * * — every 5 min  ·  0 9 * * 1-5 — 9am weekdays  ·  0 0 * * * — midnight daily';
            ...
            <div className="p-3 space-y-3 flex-1">
                ...
                {type === 'trigger' && (
                    <>
                        ...
                        <Field label="Initial Data (JSON)">
                            <Textarea value={d.initial_data || '{}'} onChange={e => set('initial_data', e.target.value)}
                                rows={4} className="text-xs font-mono resize-none" placeholder="{}" />
                        </Field>
                    </>
                )}
                ...
                {type === 'cron_trigger' && (
                    <>
                        <Field label="Label">
                            <Input value={d.label || ''} onChange={e => set('label', e.target.value)}
                                placeholder="Scheduled" className="h-7 text-xs" />
                        </Field>
                        <Field label="Cron Schedule" hint={CRON_EXAMPLES}>
                            <Input value={d.schedule || ''} onChange={e => set('schedule', e.target.value)}
                                placeholder="0 * * * *" className="h-7 text-xs font-mono" />
                        </Field>
                        <Field label="Initial Data (JSON)" hint="Injected as $input on each run">
                            <Textarea value={d.initial_data || '{}'} onChange={e => set('initial_data', e.target.value)}
                                rows={3} className="text-xs font-mono resize-none" placeholder="{}" />
                        </Field>
                    ...
                    <>
                        <Field label="URL" hint={INTERP_HINT}>
                            <Input value={d.url || ''} onChange={e => set('url', e.target.value)}
                                placeholder="https://api.example.com/{{id}}" className="h-7 text-xs" />
                        </Field>
                        <Field label="Method">
                        ...
                        <Field label="Headers (JSON)" hint={INTERP_HINT}>
                            <Textarea value={d.headers || '{}'} onChange={e => set('headers', e.target.value)}
                                rows={3} className="text-xs font-mono resize-none"
                        ...
                        <Field label="Output Key">
                            <Input value={d.output_key || 'response'} onChange={e => set('output_key', e.target.value)}
                ...
                {type === 'code' && (
                    <>
                        <Field label="JavaScript Code">
                            <Textarea value={d.code || 'return input;'} onChange={e => set('code', e.target.value)}
                                rows={8} className="text-xs font-mono resize-none" placeholder="return input;" />
                        </Field>
                        <div className="space-y-1 text-[10px] text-muted-foreground/70 bg-muted/30 rounded p-2">
                            <p>Available: <span className="font-mono text-foreground/60">input</span> · <span className="font-mono text-foreground/60">context</span></p>
                            <p className="text-green-400/70">✓ Sandboxed · Worker thread + vm · 5s timeout</p>
                        </div>
                        <Field label="Output Key">
                            <Input value={d.output_key || 'result'} onChange={e => set('output_key', e.target.value)}
                                className="h-7 text-xs font-mono" />
                        </Field>
                    </>
                )}
                ...
                {type === 'transform' && (
                    <>
                        <Field label="Expression" hint="No 'return' needed — the expression value is the output">
                            <Textarea value={d.expression || 'input'} onChange={e => set('expression', e.target.value)}
                                rows={4} className="text-xs font-mono resize-none"
                        ...
                        <Field label="Output Key">
                            <Input value={d.output_key || 'transformed'} onChange={e => set('output_key', e.target.value)}
                                className="h-7 text-xs font-mono" />
                        </Field>
                    </>
                )}
                ...
                {type === 'condition' && (
                    <>
                        <Field label="Expression (JS boolean)">
                            <Textarea value={d.expression || 'false'} onChange={e => set('expression', e.target.value)}
                                rows={4} className="text-xs font-mono resize-none"
                                placeholder="input.status === 200" />
                        </Field>
                        <div className="space-y-1 text-[10px] text-muted-foreground/70 bg-muted/30 rounded p-2">
                            <p>Available: <span className="font-mono text-foreground/60">input</span> · <span className="font-mono text-foreground/60">context</span></p>
                            <p>Write plain JS — no <span className="font-mono text-foreground/60">{'{{}}'}</span> needed</p>
                            <p className="text-emerald-400/70">✓ true path  ·  <span className="text-red-400/70">✗ false path</span></p>
                        </div>
                    </>
                )}
                ...
                {type === 'db_query' && (
                    <>
                        <Field label="SQL Query" hint={INTERP_HINT}>
                            <Textarea value={d.query || ''} onChange={e => set('query', e.target.value)}
                        ...
                        <Field label="Params (JSON array)" hint="Use ? placeholders in the query">
                            <Input value={d.params || '[]'} onChange={e => set('params', e.target.value)}
                                className="h-7 text-xs font-mono" placeholder='[{{trigger.id}}, "active"]' />
                        </Field>
                        ...
                        <Field label="Output Key">
                            <Input value={d.output_key || 'db_result'} onChange={e => set('output_key', e.target.value)}
                                className="h-7 text-xs font-mono" />
                        </Field>
                    </>
                )}
                ...
                {type === 'log' && (
                    <Field label="Message Template" hint={INTERP_HINT}>
                        <Textarea value={d.message || '{{$input}}'} onChange={e => set('message', e.target.value)}
                            rows={4} className="text-xs font-mono resize-none" placeholder="{{$input}}" />
                    </Field>
                )}
            </div>
