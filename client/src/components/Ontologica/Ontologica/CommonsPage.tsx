...
function StatsBar({ stats }: { stats: CommonsStats | null }) {
  ...
  const items = [
    ...
    { label: "Candidates", value: stats.totalCandidates, color: "text-amber-500" },
    { label: "Promoted", value: stats.promotedCount, color: "text-green-500" },
...
function LayersTab({
  ...
  const [selectedLayer, setSelectedLayer] = useState<LayerWithItems | null>(null);
  ...
  }, [selectedLayer, itemType, itemName, itemUri, itemDesc, loadLayerDetails]);

  const deleteItem = useCallback(
    async (layerId: number, itemId: number) => {
      try {
        await apiFetch(`/${layerId}/items/${itemId}`, { method: "DELETE" });
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 overflow-auto">
        ...
        {!loadingLayer && !selectedLayer && (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-2">
            ...
            <p className="text-sm">Select a layer to view its items</p>
          </div>
        )}
        ...
        {!loadingLayer && selectedLayer && (
          <div className="p-6 space-y-6">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-lg font-semibold">{selectedLayer.name}</h2>
                ...
                <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                  ...
                  {selectedLayer.dependencies.length > 0 && (
                    <span>Deps: {selectedLayer.dependencies.join(", ")}</span>
                  )}
                </div>
              </div>
            ...
            <div>
              ...
              {selectedLayer.items.length === 0 && (
                <p className="text-sm text-muted-foreground">No items in this layer yet.</p>
              )}
                ...
                {selectedLayer.items.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <Badge variant="secondary" className={cn("text-[10px] shrink-0", PATTERN_COLORS[item.item_type])}>
                        {item.item_type}
                      </Badge>
                      <span className="text-sm font-medium truncate">{item.name}</span>
                      <span className="text-xs text-muted-foreground truncate">{item.uri}</span>
                    </div>
                    <Button
                      variant="ghost"
                      size="sm"
                      className="shrink-0 text-destructive hover:text-destructive"
                      onClick={() => deleteItem(selectedLayer.id, item.id)}
                    >
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new orthogonal commons layer with its own namespace and identity.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={createLayer} disabled={!newName.trim() || creating}>
      ...
      <Dialog open={showAddItem} onOpenChange={setShowAddItem}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add Item to {selectedLayer?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>Type</Label>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={addItem}
              disabled={!itemName.trim() || !itemUri.trim() || addingItem}
            >
              {addingItem && <Loader2 className="size-3.5 mr-1 animate-spin" />}
              Add Item
            </Button>
          </DialogFooter>
        </DialogContent>
    ...
    try {
      await apiFetch("/candidates", {
        ...
        body: JSON.stringify({
          ...
          occurrence_count: Math.max(1, parseInt(cOccurrences, 10) || 1),
        }),
      });
      setCName("");
      setCDesc("");
      ...
      <div className="flex items-center gap-2 px-6 py-3 border-b border-border/40">
        ...
        {selectedIds.size > 0 && (
          <Button size="sm" onClick={() => setShowPromote(true)}>
            <ArrowUpRight className="size-3.5 mr-1" /> Promote ({selectedIds.size})
          </Button>
        )}
      </div>
          ...
          {filtered.map((c) => (
            <div
              ...
              className={cn(
                ...
                selectedIds.has(c.id) ? "border-primary/50 bg-primary/5" : "border-border/50 bg-card",
            ...
            >
              {c.status === "candidate" && (
                ...
                >
                  {selectedIds.has(c.id) && <Check className="size-3" />}
                </button>
              )}

              <Badge variant="secondary" className={cn("text-[10px] shrink-0", PATTERN_COLORS[c.pattern_type])}>
                {c.pattern_type}
              </Badge>
              ...
              <span className="text-xs text-muted-foreground shrink-0">{timeAgo(c.last_seen)}</span>
              ...
              {c.status === "promoted" && (
                <Badge className="text-[10px] bg-green-500/10 text-green-500">promoted</Badge>
              )}
              {c.status === "rejected" && (
                <Badge className="text-[10px] bg-red-500/10 text-red-400">rejected</Badge>
              )}
            </div>
          ))}
        </div>
      ...
      <Dialog open={showPromote} onOpenChange={setShowPromote}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Select a commons layer to promote {selectedIds.size} candidate(s) into.
              Items will be created in the target layer.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              {layers.length === 0 && (
                <p className="text-xs text-muted-foreground mt-1">
                  No commons layers exist yet. Create one first.
                </p>
              )}
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={promote} disabled={!targetLayerId || promoting}>
              {promoting && <Loader2 className="size-3.5 mr-1 animate-spin" />}
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Manually register a cross-project pattern as a promotion candidate.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
          ...
          <DialogFooter>
            ...
            <Button onClick={createCandidate} disabled={!cName.trim() || creatingCandidate}>
              {creatingCandidate && <Loader2 className="size-3.5 mr-1 animate-spin" />}
              Add Candidate
            </Button>
          </DialogFooter>
        </DialogContent>
    ...
    >
      ...
      <Tabs defaultValue="layers" className="flex-1 flex flex-col overflow-hidden">
        ...
        <TabsContent value="layers" className="flex-1 overflow-hidden">
          <LayersTab layers={layers} onRefresh={refresh} loading={loading} />
        </TabsContent>
