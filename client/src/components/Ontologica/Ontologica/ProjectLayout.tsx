...
export function ProjectLayout() {
  ...
  const loadJobs = useCallback(async () => {
    const res = await fetch(`/app/api/ontologica/projects/${projectId}/jobs`);
    if (res.ok) setJobs(await res.json());
  }, [projectId]);

  const loadStats = useCallback(async () => {
    const res = await fetch(`/app/api/ontologica/projects/${projectId}/stats`);
  ...
  const extractAll = useCallback(async () => {
    setExtracting(true);
    const res = await fetch(`/app/api/ontologica/projects/${projectId}/extract`, {
      method: 'POST',
  ...
  const extractDocument = useCallback(async (docId: number) => {
  ...
  const handleExport = () => {
    window.open(`/app/api/ontologica/projects/${projectId}/export?format=turtle`, '_blank');
  };
  ...
  const isExactProjectPath = location.pathname === `/ontologica/${projectId}` || location.pathname === `/ontologica/${projectId}/`;
  ...
  return (
    <AppLayout
      ...
      subtitle={project?.domain_hint ? `Domain: ${project.domain_hint}` : project?.description || ''}
      actions={
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={() => navigate('/ontologica')}>
          ...
          <Button size="sm" onClick={extractAll} disabled={extracting || docs.length === 0}>
        ...
        <div className="px-6 border-b">
          <nav className="flex gap-0 -mb-px">
            {TABS.map(tab => {
              ...
              if (tab.id === 'graph') badge = <span className="ml-1 text-xs text-muted-foreground">({nodes.length})</span>;
              if (tab.id === 'documents') badge = <span className="ml-1 text-xs text-muted-foreground">({docs.length})</span>;
              if (tab.id === 'pipeline') badge = <span className="ml-1 text-xs text-muted-foreground">({jobs.length})</span>;
              if (tab.id === 'review' && pendingCount > 0) {
                badge = <span className="ml-1 px-1.5 py-0.5 text-xs bg-amber-500/20 text-amber-400 rounded-full">{pendingCount}</span>;
              }
