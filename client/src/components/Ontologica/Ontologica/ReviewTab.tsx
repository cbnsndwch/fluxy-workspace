...
import {
  ...
  Link2, ChevronDown, ChevronRight, ChevronUp, Filter, Info, Layers, FileText,
  Download, Upload, Loader2, Pencil, ArrowRight
} from 'lucide-react';
...
export function ReviewTab() {
  const { projectId, nodes, edges, layers, docs, stats, loadGraph, loadStats, loadProject } = useProjectContext();
  ...
  // Inline edit state
  const [editingId, setEditingId] = useState<{ type: 'node' | 'edge'; id: number } | null>(null);
  ...
  const filteredNodes = useMemo(() => {
    return pendingNodes.filter((n: any) => {
      ...
      if (typeFilter === 'examples' && n.node_type !== 'individual') return false;
  ...
  const filteredEdges = useMemo(() => {
    return pendingEdges.filter((e: any) => {
      ...
      if (typeFilter === 'classes' || typeFilter === 'examples') return false;
  ...
  const groups = useMemo((): GroupedItems[] => {
    ...
    if (extractedNodes.length > 0 || extractedEdges.length > 0) {
      result.push({ key: 'extracted', label: 'Extracted Items', layerId: null, color: '', nodes: extractedNodes, edges: extractedEdges });
    }
  ...
  const filteredEdgeIds = useMemo(() => new Set(filteredEdges.map((e: any) => e.id)), [filteredEdges]);

  // Trim selection on filter change
  const prevFilterRef = useRef({ sourceFilter, typeFilter, confidenceFilter });
  ...
  if (filtersChanged) {
    ...
    const trimmedNodes = new Set([...selectedNodes].filter(id => filteredNodeIds.has(id)));
    const trimmedEdges = new Set([...selectedEdges].filter(id => filteredEdgeIds.has(id)));
    if (trimmedNodes.size !== selectedNodes.size || trimmedEdges.size !== selectedEdges.size) {
  ...
  const visibleSelectedNodes = useMemo(() => new Set([...selectedNodes].filter(id => filteredNodeIds.has(id))), [selectedNodes, filteredNodeIds]);
  const visibleSelectedEdges = useMemo(() => new Set([...selectedEdges].filter(id => filteredEdgeIds.has(id))), [selectedEdges, filteredEdgeIds]);
  const visibleSelectedCount = visibleSelectedNodes.size + visibleSelectedEdges.size;
  const allVisibleSelected = visibleSelectedCount === totalFiltered && totalFiltered > 0;

  const clearFilters = () => { setSourceFilter('all'); setTypeFilter('all'); setConfidenceFilter('all'); };
  ...
  const deselectAll = () => { setSelectedNodes(new Set()); setSelectedEdges(new Set()); };
  ...
  const quickAction = useCallback(async (type: 'node' | 'edge', id: number, action: 'approved' | 'rejected') => {
  ...
  const startEdit = useCallback((type: 'node' | 'edge', id: number, name: string, desc: string) => {
  ...
  const saveEdit = useCallback(async () => {
    ...
    setSavingEdit(true);
    try {
      const endpoint = editingId.type === 'node'
  ...
  }, [editingId, editName, editDesc, projectId]);

  // ── Export / Import ──────────────────────────────────────────────────────

  const [exportPhase, setExportPhase] = useState<null | 'grouping' | 'rendering' | 'downloading'>(null);
  ...
  const handleShareExport = async () => {
    ...
    try {
      const res = await fetch(`/app/api/ontologica/projects/${projectId}/review-export`, {
      ...
      a.href = url;
      a.download = res.headers.get('content-disposition')?.match(/filename="(.+)"/)?.[1] || 'review.html';
      a.click();
      URL.revokeObjectURL(url);
      toast.success('Review file downloaded — send it to your customer');
    } catch (e: any) {
      if (e.name !== 'AbortError') toast.error('Failed to export review file');
    } finally {
      setExportPhase(null);
  ...
  const handleImportDecisions = () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        const res = await fetch(`/app/api/ontologica/projects/${projectId}/review-import`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        toast.success(`Imported ${result.applied} decisions (${result.skipped} skipped)`);
        onReviewComplete();
      } catch (e: any) { toast.error('Failed to import: ' + (e.message || 'Invalid file')); }
    };
    input.click();
  };
      ...
      <div className="h-full flex flex-col overflow-hidden">
        <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-8">
          <div className="text-center">
            ...
            <p className="text-sm mb-4">No items pending review. Run extraction to generate new suggestions.</p>
            <DeduplicateDialog projectId={projectId} onComplete={onReviewComplete} />
          </div>
        </div>
        {/* Show layer suggestions even when review queue is empty */}
        <LayerSuggestionsPanel projectId={projectId} onComplete={onReviewComplete} />
      </div>
    );
  }
  ...
  const renderProvenance = (sourceDocId: number | null | undefined) => {
    ...
    return (
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <span className="inline-flex items-center gap-1 text-[10px] text-muted-foreground max-w-[140px] cursor-help">
          ...
          <TooltipContent side="top" className="max-w-[300px]">
            ...
            <code className="text-xs break-all">{doc.filename}</code>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
  ...
  const renderNodeCard = (n: any, isLayerGroup: boolean, lColors: any) => {
    ...
    if (isEditing) {
      return (
        <Card key={`n-${n.id}`} className="border-violet-500/50 bg-violet-500/5">
          <CardContent className="p-3 space-y-2">
            ...
            <div className="flex items-center gap-2 justify-end">
              ...
              <Button size="sm" onClick={saveEdit} disabled={savingEdit || !editName.trim()} className="h-7 text-xs">
                {savingEdit ? <Loader2 size={12} className="animate-spin mr-1" /> : null}
                Save
              </Button>
            </div>
          </CardContent>
    ...
    return (
      <Card
        ...
        className={`group/card transition-colors ${
          selectedNodes.has(n.id) ? 'border-emerald-500/50 bg-emerald-500/5' : 'hover:bg-muted/40'
      ...
      >
        <CardContent className="p-3 space-y-1.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium truncate flex-1">{n.name}</span>
            {/* Quick actions — visible on hover */}
            <div className="hidden group-hover/card:flex items-center gap-0.5 shrink-0">
              <button
                onClick={e => { e.stopPropagation(); startEdit('node', n.id, n.name, n.description); }}
              ...
              <button
                onClick={e => { e.stopPropagation(); quickAction('node', n.id, 'approved'); }}
              ...
              <button
                onClick={e => { e.stopPropagation(); quickAction('node', n.id, 'rejected'); }}
            ...
            <span className="text-xs text-muted-foreground shrink-0 group-hover/card:hidden">{Math.round(n.confidence * 100)}%</span>
          </div>
          {n.description && (
            <p className="text-xs text-muted-foreground line-clamp-2 pl-6">{n.description}</p>
          )}
          <div className="flex items-center gap-1.5 pl-6 flex-wrap">
            <Badge variant="outline" className="text-[10px]">{n.node_type === 'individual' ? 'example' : 'category'}</Badge>
  ...
  const renderEdgeCard = (e: any, isLayerGroup: boolean, lColors: any) => {
    ...
    if (isEditing) {
      return (
        <Card key={`e-${e.id}`} className="border-violet-500/50 bg-violet-500/5">
          <CardContent className="p-3 space-y-2">
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <Input
                ...
                autoFocus
                onKeyDown={ev => { if (ev.key === 'Escape') cancelEdit(); if (ev.key === 'Enter') { ev.preventDefault(); saveEdit(); } }}
              />
              <ArrowRight size={12} />
              <span className="font-medium text-foreground">{tgt?.name || e.target_value || '?'}</span>
            </div>
            <Textarea
              value={editDesc}
    ...
    return (
      <Card
        key={`e-${e.id}`}
        className={`group/card transition-colors ${
          selectedEdges.has(e.id) ? 'border-emerald-500/50 bg-emerald-500/5' : 'hover:bg-muted/40'
        } ${isLayerGroup ? `border-l-2 ${lColors?.border || ''} bg-muted/20` : ''}`}
      >
        <CardContent className="p-3 space-y-1.5">
          <div className="flex items-center gap-2">
            ...
            <div className="hidden group-hover/card:flex items-center gap-0.5 shrink-0">
              <button
                onClick={ev => { ev.stopPropagation(); startEdit('edge', e.id, e.name || e.edge_type, e.description); }}
                className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground cursor-pointer"
                title="Edit"
              >
                <Pencil size={12} />
              ...
              <button
                onClick={ev => { ev.stopPropagation(); quickAction('edge', e.id, 'approved'); }}
                className="p-1 rounded hover:bg-emerald-500/20 text-muted-foreground hover:text-emerald-400 cursor-pointer"
                title="Approve"
              >
                <Check size={14} />
              ...
              <button
                onClick={ev => { ev.stopPropagation(); quickAction('edge', e.id, 'rejected'); }}
                className="p-1 rounded hover:bg-red-500/20 text-muted-foreground hover:text-red-400 cursor-pointer"
                title="Reject"
              >
                <X size={14} />
            ...
            <span className="text-xs text-muted-foreground shrink-0 group-hover/card:hidden">{Math.round(e.confidence * 100)}%</span>
          </div>
          {e.description && (
            <p className="text-xs text-muted-foreground line-clamp-2 pl-6">{e.description}</p>
          )}
          <div className="flex items-center gap-1.5 pl-6 flex-wrap">
      ...
      <div className="shrink-0 px-6 pt-6 pb-4 space-y-4 border-b border-border/30 bg-background">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Hover any card for quick approve/reject/edit — or select multiple with checkboxes
            </p>
          </div>
          <div className="flex items-center gap-2">
            <DeduplicateDialog projectId={projectId} onComplete={onReviewComplete} />
            <Button variant="outline" size="sm" onClick={handleImportDecisions} title="Import customer feedback file">
              <Upload size={14} className="mr-1" /> Import Feedback
            </Button>
            <Button
              variant="outline"
              size="sm"
              ...
              className={exportPhase ? 'min-w-[160px]' : ''}
            >
              {exportPhase ? (
                <>
                  <Loader2 size={14} className="mr-1 animate-spin" />
                  {exportPhase === 'grouping' && 'Organizing…'}
                  {exportPhase === 'rendering' && 'Building file…'}
                  {exportPhase === 'downloading' && 'Downloading…'}
                </>
              ) : (
                <>
                  <Download size={14} className="mr-1" /> Share for Review
                </>
              )}
            </Button>
        ...
        <div className="flex items-center gap-3 flex-wrap p-3 rounded-lg bg-muted/30 border">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <FilterChip active={typeFilter === 'classes'} onClick={() => setTypeFilter('classes')}>Categories</FilterChip>
            <FilterChip active={typeFilter === 'examples'} onClick={() => setTypeFilter('examples')}>Examples</FilterChip>
        ...
        {visibleSelectedCount > 0 && (
          <div className="flex items-center gap-2 p-3 rounded-lg bg-muted/50 border">
            <span className="text-sm">{visibleSelectedCount} selected</span>
            ...
            <Button size="sm" variant="ghost" className="text-muted-foreground ml-auto" onClick={deselectAll}>
              Clear selection
            </Button>
          </div>
        )}
          ...
          return (
            <div key={group.key} className="space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="sm" className="text-xs h-6 px-2" onClick={() => selectGroup(group)}>
                  Select Group
                </Button>
              </div>
              ...
              {!isCollapsed && (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-2">
                  {group.nodes.map((n: any) => renderNodeCard(n, isLayerGroup, lColors))}
                  {group.edges.map((e: any) => renderEdgeCard(e, isLayerGroup, lColors))}
                </div>
              )}
            </div>
          );
