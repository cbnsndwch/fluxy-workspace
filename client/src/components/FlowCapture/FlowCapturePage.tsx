...
function registerMermaid(monaco: typeof MonacoNS) {
    ...
    monaco.languages.setMonarchTokensProvider('mermaid', {
        tokenizer: {
            root: [
                [/^\s*(?:flowchart|graph|sequenceDiagram|classDiagram|stateDiagram(?:-v2)?|gantt|pie|erDiagram|journey|gitGraph|mindmap|block|quadrantChart|xychart-beta)(?=\s|$)/, 'keyword.diagram'],
    ...
    monaco.editor.defineTheme('mermaid-dark', {
        ...
        inherit: true,
        rules: [
            { token: 'keyword.diagram', foreground: 'c084fc', fontStyle: 'bold' },
...
export default function FlowCapturePage() {
    ...
    const [selectedChunkSeq, setSelectedChunkSeq] = useState<number | null>(null);
    ...
    const loadSession = useCallback(async (id: number) => {
        try {
            ...
            const res = await fetch(`/app/api/flow-capture/sessions/${id}`, { credentials: 'include' });
            if (!res.ok) return;
            const { chunks: loadedChunks, diagram: loadedDiagram, chunkNodeMap: loadedMap, ...session } = await res.json();
    ...
    const generateAutoTitle = useCallback(async () => {
        ...
        try {
            const res = await fetch(`/app/api/flow-capture/sessions/${sid}/title`, {
                method: 'POST',
                credentials: 'include',
            });
            if (res.ok) {
    ...
    const generateDiagram = useCallback(async (remix = false) => {
        const sid = sessionIdRef.current;
        if (!sid) return;
        ...
        setSelectedChunkSeq(null);
        try {
            const res = await fetch(`/app/api/flow-capture/sessions/${sid}/diagram`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
                body: JSON.stringify({
                    currentDiagram: remix ? undefined : (diagramRef.current === INITIAL_DIAGRAM ? undefined : diagramRef.current),
                    remix,
                }),
            });
            if (!res.ok) throw new Error('failed');
            const { diagram: newDiagram, chunkNodeMap: newMap } = await res.json();
            if (newDiagram) {
    ...
    const saveDiagramSource = useCallback(async (code: string) => {
        const sid = sessionIdRef.current;
        if (!sid) return;
    ...
    const saveChunk = useCallback(async (text: string) => {
        ...
        try {
            const res = await fetch(`/app/api/flow-capture/sessions/${sid}/chunks`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
            ...
            setActiveSession((s) => s ? { ...s, chunk_count: s.chunk_count + 1 } : s);
            pendingChunkIdsRef.current.add(chunk.id);
            scheduleDiagramUpdate();
        } catch { /* silent */ }
    }, [scheduleDiagramUpdate]);

    const deleteChunk = useCallback(async (chunk: Chunk) => {
        const sid = sessionIdRef.current;
        if (!sid) return;
        await fetch(`/app/api/flow-capture/sessions/${sid}/chunks/${chunk.id}`, {
            method: 'DELETE',
            credentials: 'include',
        });
        setChunks((prev) => prev.filter((c) => c.id !== chunk.id));
        setActiveSession((s) => s ? { ...s, chunk_count: Math.max(0, s.chunk_count - 1) } : s);
        trackAction('segment_deleted');
    }, [trackAction]);

    const updateChunk = useCallback(async (chunk: Chunk, text: string) => {
        const sid = sessionIdRef.current;
        if (!sid) return;
        await fetch(`/app/api/flow-capture/sessions/${sid}/chunks/${chunk.id}`, {
            method: 'PATCH',
        ...
        setChunks((prev) => prev.map((c) => c.id === chunk.id ? { ...c, text } : c));
        scheduleDiagramUpdate();
    ...
    const importText = useCallback(async () => {
        ...
        try {
            ...
            setTextInput('');
            trackAction('segment_added', { source: 'text', count: toSave.length });
        } finally {
    ...
    const analyzeBuffer = useCallback(async (text: string) => {
        ...
        setIsAnalyzing(true);
        try {
            const res = await fetch(`/app/api/flow-capture/sessions/${sid}/analyze`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
                body: JSON.stringify({ text }),
            ...
            if (!res.ok) return;
            const { complete, remainder } = await res.json() as { complete: string[]; remainder: string };
    ...
    const startListening = useCallback(() => {
        ...
        recognition.onresult = (event) => {
            ...
            if (finalPart.trim()) {
                setInterimText('');
                const newBuffer = (voiceBufferRef.current + ' ' + finalPart).trim();
    ...
    const downloadSvg = useCallback(() => {
        ...
        trackAction('diagram_svg_downloaded', { sessionName: activeSession?.name });
    ...
    const highlightedSvg = useMemo(() => {
        ...
        const styleRules = nodeIds.map((id) =>
        ...
        return renderedSvg.replace('</svg>', `<style>${styleRules}</style></svg>`);
    ...
    const editableTitle = (
        <div className="flex items-center gap-1.5">
            {isEditingTitle ? (
                ...
                <button
                    onClick={() => { setTitleDraft(activeSession?.name ?? ''); setIsEditingTitle(true); }}
            ...
            {isGeneratingTitle && <Loader2 size={9} className="animate-spin text-muted-foreground" />}
        </div>
    );
    ...
    return (
        ...
        >
            <div className="flex h-full overflow-hidden">
                ...
                <div className="w-72 flex-shrink-0 flex flex-col border-r min-h-0 overflow-hidden">
                    ...
                    <div ref={chunksScrollRef} className="flex-1 min-h-0 overflow-y-auto overscroll-contain">
                        <div className="p-3 space-y-2">
                                ...
                                <div className="flex flex-col items-center justify-center gap-3 py-10 text-center px-2">
                                    ...
                                    {inputMode === 'voice' && isListening && (
                                        <p className="text-xs text-muted-foreground/60 leading-relaxed">
                                            Complete ideas are extracted automatically — fragments stay in the input below.
                                        </p>
                                    )}
                                </div>
                            )}
                            ...
                            {chunks.map((chunk) => {
                                ...
                                const isSelected = selectedChunkSeq === chunk.sequence;
                                return (
                                <div
                                    key={chunk.id}
                                    onClick={() => setSelectedChunkSeq(isSelected ? null : chunk.sequence)}
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between gap-1 mb-1.5">
                                        <span className={`text-[10px] font-mono uppercase tracking-wider ${
                                            isSelected ? 'text-violet-400/80' : 'text-muted-foreground/60'
                                        }`}>
                                            #{chunk.sequence} · {formatTime(chunk.created_at)}
                                        </span>
                                        <div className="flex items-center gap-1">
                                            <button
                                                onClick={(e) => { e.stopPropagation(); setEditingChunk(chunk); setEditText(chunk.text); }}
                                            ...
                                            <button
                                                onClick={(e) => { e.stopPropagation(); deleteChunk(chunk); }}
                                    ...
                                    <p className={`leading-relaxed ${isSelected ? 'text-foreground' : 'text-foreground/90'}`}>
                                        {chunk.text}
                                    </p>
                                </div>
                                );
                                    ...
                                    <div className="space-y-1">
                                        <div className="relative">
                                            <Textarea
                                                value={voiceBuffer}
                                                onChange={(e) => {
                                                    voiceBufferRef.current = e.target.value;
                                                    setVoiceBuffer(e.target.value);
                                                }}
                                                placeholder="Accumulated speech…"
                                                className="resize-none text-sm min-h-[72px] bg-background pr-8"
                                            />
                                            {isAnalyzing && (
                                                <Loader2 size={11} className="animate-spin absolute top-2 right-2 text-muted-foreground/50" />
                                            )}
                                        </div>
                                        {interimText && (
                            ...
                            <div className="p-3 flex flex-col gap-2">
                                ...
                                >
                                    {isImporting ? (
                                        ...
                                        <><FileText size={15} />Add to Flow</>
                                    )}
                                </Button>
                                <p className="text-[10px] text-muted-foreground text-center">
                                    ⌘↵ to submit · blank lines split into segments
                                </p>
                            </div>
                        )}
                    </div>
                        ...
                        {diagramView === 'preview' ? (
                            hasDiagram ? (
                                ...
                                <TransformWrapper
                                    ...
                                    centerOnInit
                                    wheel={{ step: 0.1 }}
                                    onZoomStop={() => trackAction('diagram_zoom')}
                                    onPanningStop={() => trackAction('diagram_pan')}
                                >
                                    <TransformComponent
                                ...
                                {renderError && !isProcessing && (
                                    <div className="absolute bottom-14 left-1/2 -translate-x-1/2 flex items-center gap-2.5 bg-amber-950/90 border border-amber-500/30 text-amber-300 text-xs rounded-lg px-3.5 py-2 shadow-lg backdrop-blur-sm">
                                        ...
                                        <span>Latest diagram has errors — showing previous version</span>
                                        <button
                                ...
                                <div className="flex h-full items-center justify-center">
                                    <div className="flex flex-col items-center gap-4 text-center max-w-xs">
                                        <div className="rounded-2xl bg-amber-500/10 p-5">
                                            <RefreshCw size={32} className="text-amber-400/60" />
                                        </div>
                                        <div>
                                            ...
                                            <p className="text-xs text-white/30 mt-1.5 leading-relaxed">
                                                The AI generated something it couldn't display. Try regenerating.
                                            </p>
                                        </div>
                                        <button
                                            onClick={() => generateDiagram(true)}
                                ...
                                <div className="flex h-full items-center justify-center">
                                    <div className="flex flex-col items-center gap-4 text-center max-w-xs">
                                        <div className="rounded-2xl bg-white/5 p-5">
                                            <GitBranch size={36} className="text-white/20" />
                                        </div>
                                        <div>
                                            ...
                                            <p className="text-xs text-white/25 mt-1.5 leading-relaxed">
                                                Add segments and the diagram will update automatically.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                            {isProcessing ? (
                                ...
                                <div className="flex items-center gap-1.5 text-xs text-amber-400/70">
                                    ...
                                    <span>New segments — diagram is stale</span>
                                </div>
                            ) : hasDiagram ? (
                                ...
                                <span className="text-xs text-white/20">Mermaid · AI generated</span>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                            ...
                            {diagramView === 'source' && (
                                <Button
                                    ...
                                    onClick={() => saveDiagramSource(diagramRef.current)}
                            ...
                            {hasContent && !isProcessing && (
                                <Button
                                    ...
                                    onClick={() => generateDiagram(diagramStale ? false : true)}
                                    title={diagramStale ? 'Retry diagram update' : 'Remix — regenerate fresh from scratch'}
                                >
                                    <RefreshCw size={12} />
        ...
        {/* Edit segment dialog */}
        <Dialog open={!!editingChunk} onOpenChange={(open) => { if (!open) setEditingChunk(null); }}>
            <DialogContent className="max-w-lg [&>button]:cursor-pointer">
                    ...
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        {editingChunk && (
                            <span className="text-xs font-normal text-muted-foreground ml-1">
                                #{editingChunk.sequence} · {formatTime(editingChunk.created_at)}
                            </span>
                        )}
                    </DialogTitle>
                    <DialogDescription>
                        Edit the segment text below. Changes mark the diagram as stale.
                    </DialogDescription>
                </DialogHeader>
                <Textarea
                    value={editText}
                ...
                <div className="flex items-center justify-end gap-2 pt-1">
                    <Button variant="ghost" onClick={() => setEditingChunk(null)} className="cursor-pointer">
                        Cancel
                    </Button>
                    <Button
                        onClick={() => {
                        ...
                        disabled={!editText.trim() || editText.trim() === editingChunk?.text}
                        className="cursor-pointer"
                    >
                        <Check size={14} />
                        Save
        ...
        <Dialog open={showSample} onOpenChange={setShowSample}>
            <DialogContent className="max-w-xl [&>button]:cursor-pointer">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        A realistic user-flow walkthrough. Hit <strong>Start Recording</strong> first, then read
                        at a natural pace. Pauses between sentences become separate segments.
                    </DialogDescription>
                </DialogHeader>
                ...
                <p className="text-xs text-muted-foreground pt-1">
                    💡 Speak naturally — complete ideas are auto-segmented, and the diagram updates live as you go.
                </p>
